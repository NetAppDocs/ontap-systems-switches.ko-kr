...
---
= Cisco Nexus 9808 스위치의 NX-OS 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
첫 번째 스위치(cs1)에 NX-OS를 설치하려면 새 버전의 NX-OS 및 EPLD를 설치하고 검증한 다음 클러스터의 모든 포트 상태를 확인하고 두 번째 스위치(cs2)에서 이 단계를 반복합니다.

. 스위치 CLI를 사용하여 NX-OS 및 EPLD 이미지를 스위치에 복사합니다.
+
...
+
이미지 파일을 설치하면 스위치가 재부팅될 때마다 해당 이미지가 로드됩니다.
...
----
====
. 스위치 CLI를 사용하여 스위치 재부팅 후 NX-OS의 새 버전을 확인하십시오.
+
`show version`
...
. 클러스터 LIF에서 자동 되돌리기 기능을 다시 활성화합니다.
+
[source, cli]
----
