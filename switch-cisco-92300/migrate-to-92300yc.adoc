...
summary: ONTAP 클러스터용 기존 Cisco 클러스터 스위치를 중단 없이 Cisco Nexus 92300YC 클러스터 네트워크 스위치로 마이그레이션할 수 있습니다. 
---
= Cisco 스위치에서 Cisco Nexus 92300YC 스위치로 마이그레이션
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ONTAP 클러스터용 기존 Cisco 클러스터 스위치를 중단 없이 Cisco Nexus 92300YC 클러스터 네트워크 스위치로 마이그레이션할 수 있습니다.


NOTE: 마이그레이션이 완료된 후 92300YC 클러스터 스위치에 대한 CSHM(Cluster Switch Health Monitor)을 지원하는 데 필요한 구성 파일을 설치해야 할 수도 있습니다. 보다link:../switch-cshm/cshm-overview.html["스위치 상태 모니터링(CSHM)"] .
...
* 완벽하게 기능하는 기존 클러스터입니다.
* 노드에서 Nexus 92300YC 클러스터 스위치로의 10GbE 및 40GbE 연결.
* 모든 클러스터 포트는 중단 없는 운영을 보장하기 위해 작동 상태입니다.
* Nexus 92300YC 클러스터 스위치에 적절한 NX-OS 버전과 참조 구성 파일(RCF)이 설치되었습니다.
...
* 기존 Cisco Nexus 5596UP 클러스터 스위치는 c1과 c2입니다.
* 새로운 Nexus 92300YC 클러스터 스위치는 cs1과 cs2입니다.
* 노드는 node1과 node2입니다.
* 클러스터 LIF는 각각 노드 1의 node1_clus1과 node1_clus2이고, 노드 2의 node2_clus1과 node2_clus2입니다.
* 스위치 c2는 먼저 스위치 cs2로 교체되고, 그 다음 스위치 c1은 스위치 cs1로 교체됩니다.
+
** c1을 cs1에 연결하는 임시 ISL이 cs1에 구축되었습니다.
** 그런 다음 노드와 c2 사이의 케이블이 c2에서 분리되고 cs2에 다시 연결됩니다.
...
.연결에 사용되는 포트
* 일부 포트는 Nexus 92300YC 스위치에서 10GbE 또는 40GbE로 실행되도록 구성되어 있습니다.
* 클러스터 스위치는 노드에 연결하기 위해 다음 포트를 사용합니다.
+
** 포트 e1/1-48(10/25GbE), e1/49-64(40/100GbE): Nexus 92300YC
...
* https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - 스위치"^]모든 클러스터 스위치에 지원되는 케이블에 대한 정보가 포함되어 있습니다.
* 이 절차에서 지원되는 ONTAP 및 NX-OS 버전은 다음과 같습니다. https://support.netapp.com/NOW/download/software/cm_switches/["Cisco 이더넷 스위치"^] 페이지.
...
----
====
. 모든 클러스터 LIF에서 자동 되돌리기가 활성화되어 있는지 확인하세요.
+
...
. 다음 명령을 사용하여 각 노드의 클러스터 포트가 기존 클러스터 스위치에 다음과 같은 방식으로 연결되었는지 확인합니다(노드 관점에서):
+
...
----
====
. 다음 명령을 사용하여 클러스터 포트와 스위치가 (스위치 관점에서) 다음과 같은 방식으로 연결되었는지 확인합니다.
+
`show cdp neighbors`
...
. c1과 cs1 사이의 포트 e1/41-48에 임시 ISL을 구성합니다.
+
.예를 보여주세요
[%collapsible]
====
다음 예에서는 c1과 cs1에서 새로운 ISL이 어떻게 구성되는지 보여줍니다.
...
----
====
. c2의 e1/41-48 포트에서 ISL 케이블을 제거하고 해당 케이블을 cs1의 e1/41-48 포트에 연결합니다.
. ISL 포트와 포트 채널이 c1과 cs1을 연결하여 작동하는지 확인합니다.
+
...
[%collapsible]
====
다음 예에서는 Cisco show port-channel summary 명령을 사용하여 c1 및 cs1에서 ISL 포트가 작동하는지 확인하는 방법을 보여줍니다.
...
----
====
. 노드 1의 경우, c2의 e1/1에서 케이블을 분리한 다음, Nexus 92300YC에서 지원하는 적절한 케이블을 사용하여 케이블을 cs2의 e1/1에 연결합니다.
. 노드 2의 경우, c2의 e1/2에서 케이블을 분리한 다음, Nexus 92300YC에서 지원하는 적절한 케이블을 사용하여 케이블을 cs2의 e1/2에 연결합니다.
...
----
====
. 노드 1의 경우, c1의 e1/1에서 케이블을 분리한 다음, Nexus 92300YC에서 지원하는 적절한 케이블을 사용하여 케이블을 cs1의 e1/1에 연결합니다.
. 노드 2의 경우, c1의 e1/2에서 케이블을 분리한 다음 Nexus 92300YC에서 지원하는 적절한 케이블을 사용하여 케이블을 cs1의 e1/2에 연결합니다.
. 각 노드의 클러스터 포트는 이제 노드 관점에서 다음과 같은 방식으로 클러스터 스위치에 연결됩니다.
+
`network device-discovery show -protocol cdp`
...
--
====
. [[3단계]] 자동 케이스 생성을 억제한 경우 AutoSupport 메시지를 호출하여 다시 활성화하세요.
+
