...
* 이 절차의 예에서는 두 개의 노드를 사용합니다. 이들 노드는 2개의 10GbE 클러스터 인터커넥트 포트를 사용합니다 `e0a` 및 `e0b`. 를 참조하십시오 https://["Hardware Universe"^] 사용 중인 플랫폼에서 올바른 클러스터 포트를 확인하려면 다음을 수행합니다.
...
* 의 스위치 호환성 표를 참조하십시오 https://["Cisco 이더넷 스위치"^] 지원되는 ONTAP, NX-OS 및 RCF 버전에 대한 페이지입니다.
