...
summary: 클러스터 운영을 위해 Cisco Nexus 92300YC 클러스터 스위치에 NX-OS 소프트웨어를 설치하십시오. 
---
= Cisco Nexus 92300YC 클러스터 스위치에 NX-OS 소프트웨어 설치
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 92300YC 클러스터 스위치에 NX-OS 소프트웨어를 설치하여 클러스터 운영을 준비하십시오. 스위치를 구성하거나 RCF를 설치하기 전에 NX-OS를 먼저 설치해야 합니다.
...
* NetApp 지원 사이트에서 스위치에 적용 가능한 NetApp Cisco NX-OS 소프트웨어는 다음에서 확인할 수 있습니다. http://mysupport.netapp.com/["mysupport.netapp.com"]
...
이 절차의 예제에서는 두 개의 노드를 사용하지만, 클러스터에는 최대 24개의 노드를 구성할 수 있습니다.

.예시에 관하여
이 절차의 예에서는 다음 스위치 및 노드 명명법을 사용합니다.

* Nexus 92300YC 스위치의 이름은  `cs1`와  `cs2`입니다.
* 이 절차에서 사용된 예시는 두 번째 스위치인 _*cs2*_에서 업그레이드를 시작합니다.
* 클러스터 LIF 이름은 다음과 같습니다. `node1_clus1` 그리고 `node1_clus2` node1의 경우 `node2_clus1` 그리고 `node2_clus2` 노드2의 경우.
* IPspace 이름은 `Cluster`입니다.
...
.다음은 무엇인가요?
NX-OS 소프트웨어를 설치한 후 link:install-the-rcf-file.html["RCF를 설치하다"]할 수 있습니다.
