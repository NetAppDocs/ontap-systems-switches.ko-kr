...
summary: 기존에 2노드 스위치가 없는 클러스터 환경을 구축한 경우, Cisco Nexus 92300YC 스위치를 사용하여 2노드 스위치 클러스터 환경으로 마이그레이션하여 클러스터의 노드 수를 2개 이상으로 확장할 수 있습니다. 
---
= Cisco Nexus 92300YC 스위치를 사용하여 2노드 스위치 클러스터로 마이그레이션
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
기존에 스위치가 없는 2노드 클러스터 환경을 보유하고 있는 경우, Cisco Nexus 92300YC 스위치를 사용하여 스위치가 있는 2노드 클러스터 환경으로 마이그레이션하여 클러스터에서 2개 이상의 노드로 확장할 수 있습니다.

사용하는 절차는 각 컨트롤러에 전용 클러스터 네트워크 포트가 두 개 있는지 또는 하나만 있는지에 따라 달라집니다. 문서화된 절차는 광 또는 트윈액스 포트를 사용하는 모든 노드에서 작동하지만, 노드가 클러스터 네트워크 포트로 온보드 10Gb BASE-T RJ45 포트를 사용하는 경우에는 이 스위치에서 지원되지 않습니다.

대부분의 시스템은 각 컨트롤러에 두 개의 전용 클러스터 네트워크 포트가 필요합니다.


NOTE: 마이그레이션이 완료되면 92300YC 클러스터 스위치용 CSHM(Cluster Switch Health Monitor)을 지원하기 위해 필요한 구성 파일을 설치해야 할 수 있습니다. link:../switch-cshm/cshm-overview.html["스위치 상태 모니터링(CSHM)"]을 참조하십시오.
...
* Nexus 92300YC 노드 간 및 스위치 간 연결에는 트윈액스 케이블 또는 광섬유 케이블이 사용됩니다.
+
link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - 스위치"^]에는 케이블 연결에 대한 자세한 정보가 포함되어 있습니다.

* ISL(Inter-Switch Link) 케이블은 두 92300YC 스위치의 1/65번 포트와 1/66번 포트에 연결됩니다.
* 92300YC 스위치 두 대 모두 초기 설정이 완료되었습니다. 따라서 다음과 같습니다.
+
** 92300YC 스위치는 최신 버전의 소프트웨어를 실행 중입니다.
** 참조 구성 파일(RCF)은 스위치에 적용됩니다. SMTP, SNMP, SSH와 같은 모든 사이트 사용자 지정은 새 스위치에서 구성됩니다.
...
[%collapsible]
====
다음 예시는 스위치 cs1에서 노드 연결 포트 1~64가 비활성화되어 있음을 보여줍니다.
...
----
====
. 두 92300YC 스위치(cs1 및 cs2) 사이의 ISL과 ISL의 물리적 포트(포트 1/65 및 1/66)가 활성화되어 있는지 확인하십시오.
+
...
----
다음 예시는 스위치 cs2에서 ISL 포트가 활성화되어 있음을 보여줍니다.
...
----
다음 예는 스위치 cs2의 인접 장치 목록을 보여줍니다.
...
+
각 클러스터 LIF는 다음과 같이 표시되어야 합니다. `Is Home` 그리고 가지고있다 `Status Admin/Oper` 위로/위로
...
----
====
. 노드1의 클러스터 포트 e0a에서 케이블을 분리한 다음, 92300YC 스위치에서 지원하는 적절한 케이블을 사용하여 e0a를 클러스터 스위치 cs1의 포트 1에 연결합니다.
+
link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - 스위치"^]에는 케이블 연결에 대한 자세한 정보가 포함되어 있습니다.

. 노드2의 클러스터 포트 e0a에서 케이블을 분리한 다음, 92300YC 스위치에서 지원하는 적절한 케이블을 사용하여 e0a를 클러스터 스위치 cs1의 포트 2에 연결합니다.
. 클러스터 스위치 cs1에서 모든 노드 연결 포트를 활성화합니다.
+
.예를 보여주세요
[%collapsible]
====
다음 예시는 스위치 cs1에서 포트 1/1부터 1/64까지가 활성화되어 있음을 보여줍니다.
...
----
====
. 모든 클러스터 LIF가 활성화되어 있고 정상적으로 작동하며 다음 항목이 모두 true로 표시되는지 확인하십시오 `Is Home`:
+
`network interface show -vserver Cluster`
...
----
====
. 노드1의 클러스터 포트 e0b에서 케이블을 분리한 다음, 92300YC 스위치에서 지원하는 적절한 케이블을 사용하여 e0b를 클러스터 스위치 cs2의 포트 1에 연결합니다.
. 노드2의 클러스터 포트 e0b에서 케이블을 분리한 다음, 92300YC 스위치에서 지원하는 적절한 케이블을 사용하여 e0b를 클러스터 스위치 cs2의 포트 2에 연결합니다.
. 클러스터 스위치 cs2에서 모든 노드 연결 포트를 활성화합니다.
+
.예를 보여주세요
[%collapsible]
====
다음 예시는 스위치 cs2에서 포트 1/1부터 1/64까지가 활성화되어 있음을 보여줍니다.
...
--
====
. [[8단계]] 자동 케이스 생성을 억제한 경우 AutoSupport 메시지를 호출하여 다시 활성화하십시오.
+
