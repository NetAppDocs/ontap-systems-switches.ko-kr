...
summary: '초기 설정 후 또는 최신 버전이 출시되면 Cisco Nexus 92300YC 클러스터 스위치에 참조 구성 파일(RCF)을 설치하거나 업그레이드하십시오.' 
---
= Cisco Nexus 92300YC 클러스터 스위치에 RCF 설치 또는 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
초기 설정 후 Cisco Nexus 92300YC 클러스터 스위치에 참조 구성 파일(RCF)을 설치하거나, 최신 버전이 출시되면 RCF 버전을 업그레이드하십시오.
...
* 클러스터 LIF 이름은  `node1_clus1`,  `node1_clus2`,  `node2_clus1`,  `node2_clus2`입니다.
* 그만큼 `cluster1::*>` 프롬프트는 클러스터의 이름을 나타냅니다.


[NOTE]
====
* 이 절차에는 ONTAP 명령과  https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["Cisco Nexus 9000 시리즈 스위치"^] 명령이 모두 필요하며, 달리 명시되지 않는 한 ONTAP 명령이 사용됩니다.
...
CAUTION: RCF를 업데이트하거나 새 RCF를 적용할 때는 스위치 설정을 지우고 기본 구성을 수행해야 합니다. 스위치를 다시 설정하려면 스위치 시리얼 콘솔 포트에 연결되어 있어야 합니다.
...
. FTP, TFTP, SFTP 또는 SCP와 같은 전송 프로토콜 중 하나를 사용하여 RCF를 스위치 cs2의 bootflash에 복사합니다. Cisco 명령에 대한 자세한 내용은 link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["Cisco Nexus 9000 시리즈 스위치"^]의 해당 가이드를 참조하십시오.
+
...
+
Cisco 명령에 대한 자세한 내용은 link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["Cisco Nexus 9000 시리즈 스위치"^]의 해당 가이드를 참조하십시오.

+
이 예시는 스위치 cs2에 설치되는 RCF 파일 `Nexus_92300YC_RCF_v1.0.2.txt`을 보여줍니다:
...
. [[step12]]RCF 파일이 올바른 최신 버전인지 확인합니다.
`show running-config`
+
...
. 이전에 적용했던 스위치 구성을 다시 적용합니다. 추가 변경 사항에 대한 자세한 내용은 link:cabling-considerations-92300.html["케이블링 및 구성 고려 사항 검토"]을 참조하십시오.
...
+
.. 클러스터의 모든 노드에서 e0d 포트가 활성화되어 있고 정상 상태인지 확인하십시오.
`network port show -ipspace Cluster`
+
.예를 보여주세요
[%collapsible]
...
----
. 클러스터 LIF가 스위치 cs2에서 호스팅되는 포트로 마이그레이션되었는지 확인하십시오. 몇 초 정도 소요될 수 있습니다. `network interface show -vserver Cluster`
+
.예를 보여주세요
[%collapsible]
====
...
.
----
. cs1과 cs2 사이의 ISL이 정상적으로 작동하는지 확인하십시오.
