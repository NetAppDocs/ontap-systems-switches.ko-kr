...
summary: 결함이 있는 NVIDIA SN2100 스토리지 스위치를 교체합니다. 이 작업은 시스템 중단 없이 진행됩니다. 
---
...
[role="lead"]
스토리지 네트워크 작동을 중단하지 않고 결함이 있는 NVIDIA SN2100 스토리지 스위치를 교체합니다. 이 무중단 절차는 교체 스위치를 준비하고 스토리지 네트워크 구성을 복원하는 과정을 포함합니다.

.시작하기 전에
NVIDIA SN2100 스토리지 스위치에 Cumulus 소프트웨어 및 RCF를 설치하기 전에 다음 사항을 확인하십시오.

* 귀하의 시스템은 NVIDIA SN2100 스토리지 스위치를 지원할 수 있습니다.
* 해당 RCF를 다운로드했습니다.


link:http://hwu.netapp.com["Hardware Universe"^]에서는 지원되는 포트 및 해당 구성에 대한 자세한 정보를 제공합니다.
