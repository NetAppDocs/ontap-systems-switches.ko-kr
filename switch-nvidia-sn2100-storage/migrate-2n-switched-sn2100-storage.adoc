...
summary: NVIDIA SN2100 클러스터 스위치를 사용하여 기존의 2노드 스위치가 없는 클러스터를 스위치 클러스터 환경으로 마이그레이션합니다. 
---
...
[role="lead"]
NVIDIA SN2100 클러스터 스위치를 사용하여 기존의 2노드 스위치가 없는 클러스터를 스위치드 클러스터 환경으로 마이그레이션하십시오. 이 무중단 마이그레이션을 통해 클러스터를 2개 이상의 노드로 확장할 수 있습니다.
...
NVIDIA SN2100 클러스터 스위치 구성의 경우 다음 사항을 확인하세요.
...
NOTE: 주의사항 및 자세한 내용은 link:cabling-considerations-sn2100-storage.html["케이블링 및 구성 고려 사항"]을(를) 참조하십시오.
...
----
+ 다음 예에서는 ISL 포트가 스위치 sw2에서 작동 중임을 보여줍니다.
...
. 스위치 sw1에서 모든 노드 방향 포트를 활성화합니다.
+
.예를 보여주세요
[%collapsible]
====
다음 명령은 스위치 sw1에서 모든 노드 연결 포트를 활성화합니다.
...
----
====
. 두 스위치 sw1과 sw2에서 두 노드가 각각 스위치에 대한 연결을 하나씩 가지고 있는지 확인하세요.
+
`net show lldp`
...
--
====
. [[step8]] 다음 명령을 사용하여 스위치 관련 로그 파일을 수집하기 위한 이더넷 스위치 상태 모니터 로그 수집 기능을 활성화합니다.
+
...
+
10분간 기다린 후 다음 명령을 사용하여 로그 수집이 성공적으로 완료되었는지 확인하세요.
