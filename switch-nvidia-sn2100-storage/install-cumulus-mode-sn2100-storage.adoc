...
summary: NVIDIA SN2100 스토리지 스위치가 Cumulus 모드로 실행 중일 때 Cumulus Linux를 설치하십시오. 
---
= NVIDIA SN2100 스토리지 스위치에 Cumulus Linux를 Cumulus 모드로 설치
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVIDIA SN2100 스토리지 스위치가 Cumulus 모드로 실행 중일 때 Cumulus Linux(CL)를 설치합니다.


NOTE: Cumulus Linux(CL) OS는 스위치가 Cumulus Linux 또는 ONIE를 실행 중일 때 설치할 수 있습니다(link:install-onie-mode-sn2100-storage.html["ONIE 모드로 설치"] 참조).
...
----
+
자세한 내용은 기술 자료 문서 link:https://kb.netapp.com/on-prem/Switches/Nvidia-KBs/NTP_Server_configuration_is_not_working_with_NVIDIA_SN2100_Switches["NTP 서버 구성이 NVIDIA SN2100 스위치에서 작동하지 않습니다."^]를 참조하십시오.
...
----
+
자세한 내용은 link:https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-54/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA 사용자 계정"^]을 참조하십시오.
...
자세한 내용은 link:https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-511/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA 사용자 계정"^]을 참조하십시오.
