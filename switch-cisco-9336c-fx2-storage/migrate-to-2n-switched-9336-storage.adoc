...
summary: 기존의 2노드 스위치가 없는 클러스터를 Cisco Nexus 9336C-FX2 및 9336C-FX2-T 스토리지 스위치를 사용하는 스위치 클러스터 환경으로 마이그레이션합니다. 
---
= Cisco Nexus 9336C-FX2 스토리지 스위치를 사용하여 2노드 스위치 클러스터로 마이그레이션
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
기존의 2노드 스위치가 없는 클러스터를 Cisco Nexus 9336C-FX2 및 9336C-FX2-T 스토리지 스위치를 사용하는 스위치 클러스터 환경으로 마이그레이션합니다. 이 무중단 마이그레이션을 통해 클러스터를 2개 이상의 노드로 확장할 수 있습니다.
...
* 보다 link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - 스위치"^] 케이블링에 대한 자세한 내용은 다음을 참조하십시오.
* 보다 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU에 없는 추가 정보 중 장비를 설치하는 데 필요한 정보는 무엇입니까?"^] 스위치 설치 요구 사항에 대한 자세한 내용은 다음을 참조하십시오.
...
보다 https://hwu.netapp.com["Hardware Universe"^] 플랫폼에 대한 클러스터 포트에 대한 정보는 다음을 참조하세요.
...
. ONTAP 9.8 이상의 경우 다음 명령을 사용하여 스위치 관련 로그 파일을 수집하는 이더넷 스위치 상태 모니터 로그 수집 기능을 활성화합니다.
+
`system switch ethernet log setup-password`그리고 `system switch ethernet log enable-collection`
...
. ONTAP 릴리스 9.5P16, 9.6P12, 9.7P10 및 이후 패치 릴리스의 경우 다음 명령을 사용하여 스위치 관련 로그 파일을 수집하는 이더넷 스위치 상태 모니터 로그 수집 기능을 활성화합니다.
+
`system cluster-switch log setup-password`그리고 `system cluster-switch log enable-collection`
