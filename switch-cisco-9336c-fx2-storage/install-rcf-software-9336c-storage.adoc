...
summary: '초기 설정 후 포트 할당 및 네트워크 설정을 구성하려면 Cisco Nexus 9336C-FX2 및 9336C-FX2-T 스토리지 스위치에 참조 구성 파일(RCF)을 설치하십시오.' 
---
= Cisco Nexus 9336C-FX2 및 9336C-FX2-T 스토리지 스위치에 RCF를 설치합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
초기 설정 후 Cisco Nexus 9336C-FX2 및 9336C-FX2-T 스토리지 스위치에 참조 구성 파일(RCF)을 설치하십시오. RCF는 스토리지 네트워크 작동에 필요한 포트 할당 및 네트워크 설정을 정의합니다.

RCF 설치에 대한 자세한 내용은 기술 자료 문서 link:https://kb.netapp.com/on-prem/Switches/Cisco-KBs/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["원격 연결을 유지하면서 Cisco 상호 연결 스위치의 구성을 지우는 방법"^]를 참조하십시오.

.시작하기 전에
다음 설치 및 연결을 확인하세요.

* 스위치에 대한 콘솔 연결.  스위치에 원격으로 접속할 수 있는 경우 콘솔 연결은 선택 사항입니다.
* 스위치 s1과 스위치 s2의 전원이 켜졌고 초기 스위치 설정이 완료되었습니다(관리 IP 주소 및 SSH가 설정됨).
* 원하는 NX-OS 버전이 설치되었습니다.
* ONTAP 노드 스토리지 포트와 스토리지 쉘프 포트가 연결되어 있지 않습니다.
...
[%collapsible]
====
이 예에서는 스위치 s2에 RCF `NX9336C-FX2-RCF-v1.13-1-Storage.txt`가 설치되는 것을 보여줍니다.
...
. ONTAP 클러스터의 모든 노드의 노드 스토리지 포트와 스토리지 쉘프 포트를 스위치 s1과 s2에 연결합니다.
