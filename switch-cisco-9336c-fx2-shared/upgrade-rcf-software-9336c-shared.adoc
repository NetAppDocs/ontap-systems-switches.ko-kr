...
summary: 'Cisco Nexus 9336C-FX2 공유 스위치에서 새로운 버전의 참조 구성 파일(RCF)이 출시되면 이를 업그레이드하십시오.' 
---
= Cisco Nexus 9336C-FX2 공유 스위치의 RCF 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 9336C-FX2 공유 스위치에서 새로운 버전의 참조 구성 파일(RCF)이 제공되면 해당 파일을 업그레이드하십시오.
...
----
====
. 각 클러스터 포트, 노드 스토리지 포트 및 스토리지 쉘프 포트의 관리 및 운영 상태를 확인하십시오.
+
...
. 자세한 내용은 link:cabling-considerations-9336c-shared.html["케이블링 및 구성 고려 사항 검토"]를 참조하십시오.
...
====
.다음은 무엇인가요?
RCF를 업그레이드한 후에는 다음을 수행할 수 있습니다. link:../switch-cshm/config-overview.html["스위치 상태 모니터링 구성"].
