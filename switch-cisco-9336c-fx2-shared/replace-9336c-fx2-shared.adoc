...
[role="lead"]
결함이 있는 Nexus 9336C-FX2 공유 스위치를 교체할 수 있습니다. 이 절차는 NDU(무중단 절차)입니다.
...
. [[4단계]] 모든 클러스터 LIF에 자동 복구가 설정되어 있는지 확인합니다.
+
...
. [[step5]] 원격 클러스터 인터페이스의 연결을 확인합니다.
...
. [[9단계]] 모든 노드에서 포트 e3b가 작동 중인지 확인합니다.
+
...
. [[12단계]] 모든 물리적 클러스터 포트가 작동하는지 확인합니다.
+
네트워크 포트에는 IPSpace 클러스터가 표시됩니다
...
. [[step13]] 원격 클러스터 인터페이스의 연결을 확인합니다.
...
. [[step15]] 이전 스위치 SH2에서 새 스위치 newsh2로 스토리지 포트를 이동합니다.
. HA 쌍 1에 연결된 스토리지, 공유 스위치 newsh2가 정상인지 확인합니다.
. HA 쌍 2에 연결된 스토리지, 공유 스위치 newsh2가 정상인지 확인:
+
