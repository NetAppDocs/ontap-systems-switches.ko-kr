...
keywords:  
summary: 직접 연결 스토리지를 사용하는 스위치 구성에서 두 개의 새 공유 스위치를 추가하여 Cisco Nexus 9336C-FX2 공유 스위치로 마이그레이션합니다. 
---
= 직접 연결 스토리지를 사용하는 스위치 구성에서 Cisco Nexus 9336C-FX2 공유 스위치로 마이그레이션합니다.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
직접 연결 스토리지를 사용하는 스위치 구성에서 두 개의 새 공유 스위치를 추가하여 Cisco Nexus 9336C-FX2 공유 스위치로 마이그레이션합니다.
...
이 절차에서 지원되는 ONTAP 및 NX-OS 버전은 Cisco 이더넷 스위치 페이지에 나와 있습니다.  https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco 이더넷 스위치"^]를 참조하십시오.
...
link:https://hwu.netapp.com["Hardware Universe"^]에는 모든 클러스터 스위치에서 지원되는 케이블에 대한 정보가 포함되어 있습니다.
