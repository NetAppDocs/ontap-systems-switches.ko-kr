...
summary: 기존에 2노드 스위치리스 클러스터 환경이 있는 경우 Cisco Nexus 9332D-GX2B 스위치를 사용하여 2노드 스위치 클러스터 환경으로 마이그레이션하면 클러스터에서 노드를 두 개 이상으로 확장할 수 있습니다. 
---
...
[role="lead"]
기존의 2노드 _스위치리스_ 클러스터 환경이 있는 경우 Cisco Nexus 9332D-GX2B 스위치를 사용하여 2노드 _스위치_ 클러스터 환경으로 마이그레이션할 수 있습니다.
...
** 지원되는 모든 ONTAP 버전은  https://hwu.netapp.com["Hardware Universe"^]를 참조하십시오.


* Cisco Nexus 9332D-GX2B 스위치 구성의 경우:
+
** 두 스위치 모두 관리 네트워크 연결 기능을 갖추고 있습니다.
** 클러스터 스위치에 콘솔로 접근할 수 있습니다.
** Nexus 9332D-GX2B 노드 간 스위치와 스위치 간 연결에는 Twinax 또는 파이버 케이블을 사용합니다.
+
케이블 연결에 대한 자세한 내용은 https://hwu.netapp.com["Hardware Universe"^]를 참조하십시오.



* ISL(Inter-Switch Link) 케이블은 두 9332D-GX2B 스위치의 포트 1/35와 1/36에 연결됩니다.
* 두 9332D-GX2B 스위치의 초기 사용자 정의가 완료되어 다음과 같은 결과가 나왔습니다.
+
** 9332D-GX2B 스위치는 최신 버전의 소프트웨어를 실행하고 있습니다.
...
* 9332D-GX2B2 스위치의 이름은 cs1과 cs2입니다.
...
사용 중인 플랫폼의 클러스터 포트에 대한 정보는  https://hwu.netapp.com["Hardware Universe"^]을(를) 참조하십시오. 스위치 설치 요구 사항에 대한 자세한 내용은 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU에 없는 추가 정보 중 장비를 설치하는 데 필요한 정보는 무엇입니까?"^]을(를) 참조하십시오.
...
----
====
. 두 9332D-GX2B 스위치 cs1과 cs2 사이의 ISL과 ISL의 물리적 포트가 포트 1/35와 1/36에서 작동하는지 확인하세요.
+
...
----
====
. 노드 1의 클러스터 포트 e0a에서 케이블을 분리한 다음, 9332D-GX2B 스위치에서 지원하는 적절한 케이블을 사용하여 e0a를 클러스터 스위치 cs1의 포트 1에 연결합니다.
+
그만큼 https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe - 스위치"^] 케이블링에 대한 자세한 정보가 포함되어 있습니다.

. 노드2의 클러스터 포트 e0a에서 케이블을 분리한 다음, 9332D-GX2B 스위치에서 지원하는 적절한 케이블을 사용하여 e0a를 클러스터 스위치 cs1의 포트 2에 연결합니다.
...
----
====
. 노드 1의 클러스터 포트 e0b에서 케이블을 분리한 다음, 9332D-GX2B 스위치에서 지원하는 적절한 케이블을 사용하여 e0b를 클러스터 스위치 cs2의 포트 1에 연결합니다.
. 노드2의 클러스터 포트 e0b에서 케이블을 분리한 다음, 9332D-GX2B 스위치에서 지원하는 적절한 케이블을 사용하여 e0b를 클러스터 스위치 cs2의 포트 2에 연결합니다.
