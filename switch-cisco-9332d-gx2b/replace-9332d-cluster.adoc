...
summary: AFX 시스템에서 작동을 중단하지 않고 결함이 있는 Cisco Nexus 9332D-GX2B 스위치를 교체합니다. 이 작업은 시스템 중단 없이 진행됩니다. 
---
= AFX 시스템에서 Cisco Nexus 9332D-GX2B 스위치 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX 시스템에서 고장난 Cisco Nexus 9332D-GX2B 스위치를 시스템 운영 중단 없이 교체합니다. 이 비중단 교체 절차(NDU)는 교체 스위치를 준비하고 시스템 구성을 복원하는 과정을 포함합니다.
...
** 스위치의 초기 사용자 지정이 완료되었으며, 자세한 내용은 link:configure-complete-initial-setup-9332d.html["스위치 초기 설정 완료"]에 설명되어 있습니다.
+
...
* 유지보수 전후에 유지보수 AutoSupport를 실행하여 유지보수 기간 동안 케이스 생성을 비활성화하십시오. 자세한 내용은 이 기술 자료 문서 https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU92["SU92: 예약된 유지 관리 기간 동안 자동 케이스 생성을 억제하는 방법"^]를 참조하십시오.
...
. 새 스위치에서 관리자로 로그인하고 노드 클러스터 인터페이스에 연결될 모든 포트(포트 1/1~1/30)를 종료합니다.
+
...
. Nexus 9332D-GX2B 스위치 cs1에서 ISL 포트 Eth1/31 및 Eth1/32를 종료합니다.
+
[listing, subs="+quotes"]
----
cs1# *config*
...
cs1(config)# *exit*
----
. Nexus 9332D-GX2B cs2 스위치에서 모든 케이블을 제거한 다음, 이를 9332D-GX2B newcs2 스위치의 동일한 포트에 연결합니다.
. cs1과 newcs2 스위치 사이에서 ISL 포트 Eth1/31과 Eth1/32를 가동한 후 포트 채널 작동 상태를 확인합니다.
+
...
----
====
. newcs2 스위치의 모든 포트(Eth1/1부터 Eth1/30까지)를 활성화하십시오.
+
.예를 보여주세요
[%collapsible]
====
...
. 원격 클러스터 인터페이스의 연결성을 확인하세요.
+
.. 당신은 사용할 수 있습니다 `network interface check cluster-connectivity` 클러스터 연결에 대한 접근성 검사 세부 정보를 표시하는 명령:
+
[source, cli]
----
