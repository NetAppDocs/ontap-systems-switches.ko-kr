...
summary: Cisco Nexus 9332D-GX2B 스위치의 NX-OS를 업그레이드하여 AFX 1K 또는 AFX 2K 시스템의 새로운 하드웨어 또는 새로운 ONTAP 버전을 지원합니다. 
---
= Cisco Nexus 9332D-GX2B 스위치의 NX-OS 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 9332D-GX2B 스위치의 NX-OS를 업그레이드하여 AFX 1K 또는 AFX 2K 시스템의 새로운 하드웨어 또는 새로운 ONTAP 버전을 지원합니다.
...
* 필요한 경우, 현재 및 목표 릴리스를 기반으로 Cisco Nexus 9000 시리즈 스위치의 서비스 중 소프트웨어 업그레이드(ISSU)에 대한 정보를 제공하는 link:https://www.cisco.com/c/dam/en/us/td/docs/dcn/tools/nexus-9k3k-issu-matrix/index.html["Cisco Nexus 9000 및 3000 업그레이드 및 ISSU 매트릭스"^]를 참조할 수 있습니다.
+
...
첫 번째 스위치(cs1)에 NX-OS 및 EPLD 이미지를 설치하고 검증한 다음 클러스터의 모든 포트 상태를 확인하고 두 번째 스위치(cs2)에서 이 단계를 반복합니다.

. NX-OS 및 EPLD 이미지를 설치합니다. 설치 단계는 현재 사용 중인 NX-OS 버전과 설치 대상 NX-OS 버전에 따라 다를 수 있습니다.
+
본인의 상황에 맞는 탭을 선택하십시오.

+
** 10.4(2)F에서 10.6(3)F 이상으로 업그레이드
** 10.6(3)F에서 이후 버전으로 업그레이드
...
. [[단계]] NX-OS 및 EPLD 이미지를 첫 번째 스위치에 복사합니다.
+
...
[%collapsible]
=====
이 예제는 sftp와 NX-OS 버전 10.6(4)M을 사용합니다.
...
[%collapsible]
=====
이 예제는 NX-OS 버전 10.6(4)M을 사용합니다.
...
.다음은 무엇인가요?
스위치의 NX-OS를 업그레이드한 후에는 link:configure-install-upgrade-rcf-overview-9332d.html["RCF 설치 또는 업그레이드 개요"]부터 시작하여 RCF를 업그레이드합니다.
