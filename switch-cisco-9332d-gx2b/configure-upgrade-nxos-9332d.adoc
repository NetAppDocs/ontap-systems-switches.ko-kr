...
NX-OS를 첫 번째 스위치(cs1)에 설치하려면 NX-OS 및 EPLD의 새 버전을 설치하고 확인한 다음 클러스터의 모든 포트 상태를 확인하고 두 번째 스위치(cs2)에서 이 단계를 반복합니다.
...
----
====
. 스위치 CLI를 사용하여 NX-OS 이미지를 설치합니다.
+
...
----
====
. 스위치 CLI를 사용하여 다음 명령으로 EPLD 이미지를 업그레이드하십시오.
+
...
----
====
. 스위치가 재부팅된 후 스위치 CLI를 사용하여 로그인하고 새 버전의 EPLD가 성공적으로 로드되었는지 확인하십시오.
+
`show version module 1 epld`
