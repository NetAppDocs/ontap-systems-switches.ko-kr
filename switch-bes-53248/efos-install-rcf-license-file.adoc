...
summary: '초기 구성을 완료한 후 Broadcom BES-53248 클러스터 스위치에 참조 구성 파일(RCF) 및 라이센스 파일을 설치합니다.' 
---
= Broadcom BES-53248 클러스터 스위치에 RCF 파일과 라이선스 파일을 설치합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248 클러스터 스위치에 참조 구성 파일(RCF)과 라이선스 파일을 함께 설치하십시오. EFOS 3.12.0.1부터는 초기 스위치 구성을 완료한 후 두 파일을 모두 설치할 수 있습니다.


CAUTION: RCF를 설치하면 모든 포트가 구성되지만, 구성된 포트를 활성화하려면 라이선스를 설치해야 합니다.
...
* 현재 RCF는 link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Broadcom 클러스터 스위치"^] 페이지에서 확인할 수 있습니다.
...
. FTP, HTTP, TFTP, SFTP 또는 SCP 중 하나의 전송 프로토콜을 사용하여 RCF를 스위치 cs2의 부트플래시에 복사합니다.
+
이 예에서는 HTTP를 사용하여 RCF를 스위치 cs2의 부트플래시에 복사하는 방법을 보여줍니다.
...
.다음은 무엇인가요?
RCF 및 라이센스 파일을 설치한 후에는 다음을 수행할 수 있습니다. link:configure-ssh.html["SSH 활성화"].
