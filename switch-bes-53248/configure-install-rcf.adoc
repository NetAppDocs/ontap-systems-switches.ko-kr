...
summary: '초기 구성을 완료하고 필요한 라이선스를 적용한 후 Broadcom BES-53248 클러스터 스위치에 참조 구성 파일(RCF)을 설치하십시오.' 
---
= Broadcom BES-53248 클러스터 스위치에 RCF 설치
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
초기 구성 및 필요한 라이선스 적용을 완료한 후 Broadcom BES-53248 클러스터 스위치에 참조 구성 파일(RCF)을 설치하십시오. RCF는 클러스터 작동을 위한 포트 할당, VLAN 및 네트워크 설정을 정의합니다.


CAUTION: EFOS 3.12 이상 버전의 경우 link:efos-install-rcf-license-file.html["참조 구성 파일(RCF) 및 라이센스 파일을 설치합니다"]에 있는 설치 단계를 따르십시오.
...
.시작하기 전에
다음 사항이 제대로 적용되었는지 확인하세요.

* 스위치 구성의 현재 백업입니다.
* 완벽하게 작동하는 클러스터(로그에 오류나 유사한 문제가 없음).
* 현재 RCF 파일은 link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Broadcom 클러스터 스위치"^] 페이지에서 확인할 수 있습니다.
...
* RCF를 공장 초기 설정 상태에서 설치할 경우 스위치에 대한 콘솔 연결이 필요합니다. 이 요구 사항은 사전에 구성을 지우기 위해 기술 자료 문서 link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["원격 연결을 유지하면서 Broadcom 상호 연결 스위치의 구성을 지우는 방법"^]를 사용한 경우 선택 사항입니다.


.제안된 문서
지원되는 ONTAP 및 RCF 버전은 스위치 호환성 표를 참조하십시오. link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["EFOS 소프트웨어 다운로드"^]을(를) 참조하십시오. RCF의 명령 구문과 EFOS 버전에 있는 명령 구문 사이에 명령 종속성이 있을 수 있습니다.
...
* 이 절차의 예제에서는 4개의 노드를 사용합니다. 이러한 노드는 2개의 10GbE 클러스터 인터커넥트 포트 `e0a` 및 `e0b`를 사용합니다. link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]를 참조하여 플랫폼에서 올바른 클러스터 포트를 확인하십시오.
...
CAUTION: 새로운 스위치 소프트웨어 버전 및 RCF를 설치하기 전에 기술 자료 문서 link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["원격 연결을 유지하면서 Broadcom 상호 연결 스위치의 구성을 지우는 방법"^]를 참조하십시오. 스위치 설정을 완전히 삭제해야 하는 경우 기본 구성을 다시 수행해야 합니다. 전체 구성 삭제는 관리 네트워크 구성을 초기화하므로 시리얼 콘솔을 사용하여 스위치에 연결해야 합니다.
...
show isdp neighbor
----
. 클러스터 스위치 cs2에서 노드의 클러스터 포트에 연결된 포트를 종료합니다.  예를 들어, 포트 0/1~0/16이 ONTAP 노드에 연결된 경우:
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
. 다음 사항에 유의하세요.
+
.. 스위치에 추가 포트 라이센스가 설치된 경우 추가 라이센스 포트를 구성하려면 RCF를 수정해야 합니다. 자세한 내용은 link:configure-licenses.html#activate-newly-licensed-ports["새로 라이센스된 포트 활성화"]를 참조하십시오.
...
--
====
. 배너 출력을 검사합니다. `show clibanner` 명령.  스위치의 올바른 구성과 작동을 확인하려면 다음 지침을 읽고 따라야 합니다.
+
.예를 보여주세요
[%collapsible]
====
...
----
. 클러스터 스위치 cs2에서 노드의 클러스터 포트에 연결된 포트를 불러옵니다.  예를 들어, 포트 0/1~0/16이 ONTAP 노드에 연결된 경우:
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
----
. 스위치 cs1을 재부팅합니다.  이렇게 하면 클러스터 LIF가 홈 포트로 복귀합니다.  스위치가 재부팅되는 동안 노드에서 보고된 "클러스터 포트 다운" 이벤트는 무시할 수 있습니다.
+
[listing, subs="+quotes"]
----
(cs1)# *reload*
...
.다음은 무엇인가요?
RCF를 설치한 후에는 다음을 수행할 수 있습니다. link:configure-ssh.html["SSH 활성화"].
