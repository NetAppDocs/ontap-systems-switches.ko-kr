...
summary: 'Broadcom BES-53248 클러스터 스위치의 Ethernet Fabric OS(EFOS) 소프트웨어를 업그레이드하여 ONTAP와의 호환성을 유지합니다.' 
---
= Broadcom BES-53248 클러스터 스위치의 EFOS 소프트웨어 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248 클러스터 스위치의 이더넷 패브릭 OS(EFOS) 소프트웨어를 업그레이드하여 ONTAP와의 호환성을 유지하고 최신 스위치 기능 및 수정 사항을 이용하십시오.
...
.시작하기 전에
* link:https://www.broadcom.com/support/bes-switch["Broadcom 이더넷 스위치 지원"^] 사이트에서 클러스터 스위치에 적합한 Broadcom EFOS 소프트웨어를 다운로드하십시오.
...
IMPORTANT: BES-53248 클러스터 스위치를 EFOS 3.3.x.x 또는 3.4.x.x에서 EFOS 3.7.0.4 또는 3.8.0.2로 업그레이드한 후에는 ISL(Inter-Switch Links) 및 포트 채널이 *Down* 상태로 표시됩니다. 이는 예상되는 동작이며 LIF 자동 복구에 문제가 없는 한 업그레이드를 계속 진행해도 안전합니다. 자세한 내용은 기술 자료 문서 link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Systems/Fabric%2C_Interconnect_and_Management_Switches/BES-53248_Cluster_Switch_NDU_failed_upgrade_to_EFOS_3.7.0.4_and_later["BES-53248 클러스터 스위치 NDU가 EFOS 3.7.0.4 이상으로 업그레이드하지 못했습니다."^]를 참조하십시오.
...
NOTE: 이 시점에 RCF도 업그레이드하는 경우, 스위치 cs1의 EFOS를 업그레이드하기 전에 스위치 cs2의 RCF를 업그레이드하는 절차를 따르십시오. 자세한 내용은 link:upgrade-rcf.html["RCF 버전 업그레이드"]을 참조하십시오.

. [[step12]] 스위치 cs1에 대해 5단계부터 11단계까지를 반복합니다.
