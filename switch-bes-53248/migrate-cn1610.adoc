...
* BES-53248 스위치의 일부 포트는 10GbE 또는 40GbE에서 실행되도록 구성되어 있습니다.
* 노드에서 BES-53248 클러스터 스위치로의 10GbE 및 40GbE 연결이 계획, 마이그레이션 및 문서화되었습니다.
...
----
====
. 모든 클러스터 노드 연결 케이블을 이전 CS2 스위치에서 새 CS2 스위치로 이동합니다.
...
----
====
. 다음 명령을 사용하여 스위치 CS2의 관점에서 스위치 포트 연결을 확인합니다.
+
[listing, subs="+quotes"]
----
...
----
====
. 다음 명령을 사용하여 스위치 CS1의 관점에서 스위치 포트 연결을 확인합니다.
+
[listing, subs="+quotes"]
----
...
.ONTAP 9.4 이하
--
ONTAP 9.4 이전 버전의 경우 다음 명령을 사용합니다. `system cluster-switch delete -device _device-name_`
...
IMPORTANT: 로그 수집 기능을 사용하려면 ONTAP 9.12.1 이상 및 EFOS 3.8.0.2 이상을 실행해야 합니다.
