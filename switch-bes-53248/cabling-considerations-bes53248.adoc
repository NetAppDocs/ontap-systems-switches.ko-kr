...
summary: Broadcom BES-53248 클러스터 스위치를 배포하기 전에 클러스터 포트 할당, 포트 그룹 속도 제한 및 FEC 설정 등 케이블링 및 구성 제약 조건을 이해해야 합니다. 
---
= Broadcom BES-53248 클러스터 스위치의 케이블 연결 및 구성 고려 사항을 검토하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248 클러스터 스위치를 배포하기 전에 케이블링 및 구성 제약 조건을 이해해야 합니다. 주요 고려 사항에는 클러스터 포트 할당, 포트 그룹 속도 제약 조건, 25G 구리 및 광섬유 연결에 대한 순방향 오류 수정(FEC) 설정이 포함됩니다.
...
.다음은 무엇인가요?
케이블 연결 및 구성 요구 사항을 충족한 후 스위치에 link:configure-efos-software.html["EFOS 소프트웨어를 설치하세요"]을(를) 설치합니다.
