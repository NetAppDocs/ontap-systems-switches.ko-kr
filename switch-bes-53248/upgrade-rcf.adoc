...
summary: BES-53248 클러스터 스위치를 구성하고 새 라이센스를 적용한 후 RCF를 업그레이드할 수 있습니다. 
---
= RCF 버전 업그레이드
:allow-uri-read: 
:icons: font
...
[role="lead"]
BES-53248 클러스터 스위치 EFOS를 업그레이드하고 새 라이센스 키를 적용한 후 RCF를 업그레이드할 수 있습니다.
...
* BES-53248 스위치 업그레이드 및 다운그레이드 절차에 대한 전체 문서는 link:https://www.broadcom.com/support/bes-switch["브로드컴"^] 사이트에서 제공되는 해당 소프트웨어 및 업그레이드 가이드를 참조하십시오.
...
* 이 절차의 예제에서는 4개의 노드를 사용합니다. 이 노드들은 2개의 10GbE 클러스터 인터커넥트 포트 `e0a`및 `e0b`를 사용합니다. link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]를 참조하여 사용 중인 플랫폼에서 올바른 클러스터 포트를 확인하십시오.
...
. 다음 사항에 유의하세요.
+
.. 스위치에 추가 포트 라이센스가 설치된 경우 추가 라이센스 포트를 구성하려면 RCF를 수정해야 합니다. 자세한 내용은 link:configure-licenses.html#activate-newly-licensed-ports["새로 라이센스된 포트 활성화"]을 참조하십시오. 하지만 RCF 1.12 이상으로 업그레이드하면 모든 인터페이스가 사전 구성되어 있으므로 더 이상 수정이 필요하지 않습니다.
...
CAUTION: RCF 파일을 업로드하고 적용할 때 파일 이름을 `reference-config.scr`로 지정해야 합니다. 그렇지 않으면 스크립트 적용 단계가 실패합니다.

. HTTP, HTTPS, FTP, SFTP 또는 SCP 중 하나의 전송 프로토콜을 사용하여 RCF를 스위치 cs2로 복사하십시오.
+
...
----
+
자세한 내용은 link:https://docs.netapp.com/us-en/ontap/networking/revert_a_lif_to_its_home_port.html["LIF를 홈 포트로 되돌리기"]을 참조하십시오.
