...
summary: 'EFOS 소프트웨어 업그레이드 후 Broadcom BES-53248 클러스터 스위치의 참조 구성 파일(RCF)을 업그레이드하십시오.' 
---
= Broadcom BES-53248 클러스터 스위치의 RCF 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
EFOS 소프트웨어를 업그레이드하고 필요한 라이선스를 적용한 후 Broadcom BES-53248 클러스터 스위치의 참조 구성 파일(RCF)을 업그레이드하십시오.
...
* RCF를 공장 초기 설정 상태에서 설치할 경우 스위치에 대한 콘솔 연결이 필요합니다.
...
포트 구성에는 스위치 CLI와 ONTAP CLI(다음 단계에 설명됨)를 사용합니다. 첫 번째 스위치(cs2)에서 포트를 구성한 다음 두 번째 스위치(cs1)에 대해 동일한 단계를 반복합니다.
...
CAUTION: RCF를 업데이트하거나 새 RCF를 적용할 때는 스위치 설정을 지우고 기본 구성을 수행해야 합니다. 스위치 설정을 지우려면 시리얼 콘솔을 사용하여 스위치에 연결해야 합니다.
...
--
====
. [[step20]]스위치 cs1에서 1~19단계를 반복합니다.
...
.다음은 무엇인가요?
RCF를 업그레이드한 후에는 link:configure-ssh.html["로그 수집을 위해 SSH 활성화"].
