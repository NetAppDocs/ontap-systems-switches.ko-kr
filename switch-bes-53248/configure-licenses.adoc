...
summary: Broadcom BES-53248 클러스터 스위치 기본 모델은 16개의 10GbE 또는 25GbE 포트와 2개의 100GbE 포트를 지원합니다. 스위치 용량을 확장하려면 추가 포트 라이선스를 설치하십시오. 
---
= Broadcom BES-53248 클러스터 스위치용 라이선스 설치
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248 클러스터 스위치 기본 모델은 16개의 10GbE 또는 25GbE 포트와 2개의 100GbE 포트에 대한 라이선스를 제공합니다. 추가 포트 라이선스를 설치하여 스위치를 확장하면 더 많은 10GbE 또는 25GbE 클러스터 포트를 지원할 수 있습니다.
...
다음 라이센스는 BES-53248 클러스터 스위치에서 사용할 수 있습니다.

[cols="1,2,3"]
|===
| 라이센스 유형 | 라이센스 세부 정보 | 지원되는 펌웨어 버전 
...
SW-BES-53248A2-8P-1025G
 a| 
Broadcom 8포트 10G25G 라이선스 키, X190005/R
 a| 
EFOS 3.4.4.6 이상
...
NOTE: 포트 라이센스 키 파일에 대한 거래 키를 등록하려면 link:https://support.broadcom.com/web/csg/license-generation["Broadcom 지원 이더넷 스위치용 라이선스 포털"^]을(를) 참조하십시오. 자세한 내용은 기술 자료 문서 link:https://kb.netapp.com/on-prem/Switches/Broadcom-KBs/How_to_add_Additional_Port_Licensing_for_the_Broadcom_BES_53248_Switch["Broadcom BES-53248 스위치에 추가 포트 라이선싱을 추가하는 방법"^]을(를) 참조하십시오.
...
다음 표에는 BES-53248 클러스터 스위치에서 사용할 수 있는 레거시 라이선스가 나열되어 있습니다.
...
SW-BES-53248A1-G1-8P-LIC
 a| 
Broadcom 8P 10-25,2P40-100 라이선스 키, X190005/R
 a| 
EFOS 3.4.3.3 이상
...
SW-BES-53248A1-G1-16P-LIC
 a| 
Broadcom 16P 10-25,4P40-100 라이선스 키, X190005/R
 a| 
EFOS 3.4.3.3 이상
...
SW-BES-53248A1-G1-24P-LIC
 a| 
Broadcom 24P 10-25,6P40-100 라이선스 키, X190005/R
 a| 
EFOS 3.4.3.3 이상
...
SW-BES54248-40-100G-LIC
 a| 
Broadcom 6Port 40G100G 라이센스 키, X190005/R
 a| 
EFOS 3.4.4.6 이상
...
SW-BES53248-8P-10G25G-LIC
 a| 
Broadcom 8Port 10G25G 라이선스 키, X190005/R
 a| 
EFOS 3.4.4.6 이상
...
SW-BES53248-16P-1025G-LIC
 a| 
Broadcom 16포트 10G25G 라이선스 키, X190005/R
 a| 
EFOS 3.4.4.6 이상
...
SW-BES53248-24P-1025G-LIC
 a| 
Broadcom 24Port 10G25G 라이선스 키, X190005/R
 a| 
EFOS 3.4.4.6 이상
...
BES-53248 클러스터 스위치에 대한 라이선스를 설치하려면 다음 단계를 따르세요.
...
----
====
. Switch CS2에서 현재 라이센스 사용량을 확인하세요:
+
`show license`
...
. 라이센스 파일을 설치합니다.
+
더 많은 라이선스를 로드하고 다른 키 인덱스 번호를 사용하려면 이 단계를 반복합니다.

+
.예를 보여주세요
[%collapsible]
====
다음 예제에서는 SFTP를 사용하여 라이선스 파일을 키 인덱스 1로 복사합니다.
...
----
====
. switch cs2를 재부팅하기 전에 모든 현재 라이센스 정보를 표시하고 라이센스 상태를 기록합니다.
+
`show license`
...
+
추가 라이선스 파일의 포트는 스위치를 재부팅한 후에야 표시됩니다.
...
----
====
. 새 라이선스가 활성화되어 있는지 확인하고 라이선스가 적용되었는지 확인하세요.
+
`show license`
...
CAUTION: 추가 라이선스를 설치할 때는 새 인터페이스를 수동으로 구성해야 합니다.  기존의 작동 중인 생산 스위치에 RCF를 다시 적용하지 마세요.
...
라이센스 설치 시 문제가 발생하는 경우 실행하기 전에 다음 디버그 명령을 실행하십시오. `copy` 다시 명령을 내리세요.

사용할 디버그 명령: `debug transfer` 그리고 `debug license`
...
----
====
당신이 실행할 때 `copy` 명령으로 `debug transfer` 그리고 `debug license` 옵션이 활성화되면 로그 출력이 반환됩니다.
...
----
====
디버그 출력에서 다음 사항을 확인하세요.

* 일련 번호가 일치하는지 확인하세요. `Serial number QTFCU38290012 matched.`
* 스위치 모델이 다음과 일치하는지 확인하세요. `Model BES-53248 matched.`
* 지정된 라이선스 인덱스가 이전에 사용되지 않았는지 확인하세요.  라이센스 인덱스가 이미 사용된 경우 다음 오류가 반환됩니다. `License file /mnt/download//license.dat.1 already exists.`
* 포트 라이센스는 기능 라이센스가 아닙니다.  따라서 다음과 같은 진술이 예상됩니다. `Feature not found in license file with index = 1.`


사용하다 `copy` 포트 라이선스를 서버에 백업하는 명령:
...
CAUTION: 스위치 소프트웨어를 3.4.4.6 버전에서 다운그레이드해야 하는 경우 라이선스가 제거됩니다.  이는 예상되는 동작입니다.

이전 버전의 소프트웨어로 되돌리려면 먼저 적절한 이전 라이선스를 설치해야 합니다.
...
새로 허가된 포트를 활성화하려면 RCF의 최신 버전을 편집하고 해당 포트 세부 정보의 주석 처리를 제거해야 합니다.

기본 라이선스는 포트 0/1~0/16과 0/55~0/56을 활성화하는 반면, 새로 라이선스가 부여된 포트는 사용 가능한 라이선스의 유형과 수에 따라 포트 0/17~0/54 사이에 위치하게 됩니다.  예를 들어, SW-BES54248-40-100G-LIC 라이선스를 활성화하려면 RCF에서 다음 섹션의 주석 처리를 제거해야 합니다.
...
NOTE: 0/49에서 0/54까지의 고속 포트의 경우, 각 포트의 주석 처리를 제거하지만, 각 포트에 대해 RCF에서 *speed* 줄 하나만 주석 처리를 제거합니다. 예에서 볼 수 있듯이 *speed 100G full-duplex* 또는 *speed 40G full-duplex*입니다.  0/17에서 0/48까지의 저속 포트의 경우, 해당 라이선스가 활성화되면 8포트 섹션 전체의 주석 처리를 제거합니다.
