...
summary: CSHM 및 로그 수집을 지원하려면 Cisco Nexus 9336C-FX2 클러스터 스위치에서 SSH 및 SSH 키가 활성화되어 있는지 확인하십시오. 
---
= Cisco Nexus 9336C-FX2 클러스터 스위치의 SSH 구성을 확인합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 9336C-FX2 및 9336C-FX2-T 클러스터 스위치에서 SSH 및 SSH 키가 활성화되어 있는지 확인하여 이더넷 스위치 상태 모니터링(CSHM) 및 로그 수집 기능을 지원하도록 하십시오.
...
IMPORTANT: FIPS를 활성화하려면  `ssh key ecdsa 256 force` 명령을 사용하여 스위치에서 비트 수를 256으로 변경해야 합니다. 자세한 내용은 link:https://docs.netapp.com/us-en/ontap/networking/configure_network_security_using_federal_information_processing_standards_@fips@.html#enable-fips["FIPS를 사용하여 네트워크 보안 구성"^]를 참조하십시오.
