...
---
= 이전 Cisco 스위치에서 Cisco Nexus 9336C-FX2 클러스터 스위치로 마이그레이션하는 방법
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.시작하기 전에
이전 Cisco Nexus 클러스터 스위치를 Cisco Nexus 9336C-FX2 클러스터 스위치로 교체하기 전에 다음 조건이 충족되어야 합니다.

* 클러스터가 완전하게 작동해야 합니다(로그에 오류가 없거나 이와 유사한 문제가 없어야 함).
* Cisco Nexus 9336C-FX2 스위치의 초기 사용자 정의를 완료해야 합니다. 따라서 다음을 수행합니다.
+
** 9336C-FX2 스위치는 권장되는 최신 소프트웨어 버전을 실행하고 있습니다
** RCF(Reference Configuration Files)가 스위치에 적용되었습니다
** DNS, NTP, SMTP, SNMP 등의 사이트 사용자 지정 및 SSH는 새 스위치에 구성해야 합니다.
...
* 의 Cisco 스위치 업그레이드 및 다운그레이드 절차에 대한 전체 문서를 보려면 Cisco 웹 사이트에서 제공되는 적절한 소프트웨어 및 업그레이드 가이드를 참조해야 합니다 https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.htmlCisco["Nexus 9000 시리즈 스위치"^].
...
NOTE: 이 절차를 수행하려면 ONTAP 명령과 를 모두 사용해야 합니다 https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.htmlCisco["Nexus 9000 시리즈 스위치"^] 달리 지시되지 않는 한, Commands; ONTAP 명령어가 사용된다.
...
. 새 스위치에서 ISL이 케이블로 연결되고 스위치 CS1-NEW와 CS2-NEW:'포트-채널 요약 정보' 간에 양호한지 확인합니다
+
[listing, subs="+quotes"]
----
...
1     Po1(SU)     Eth      LACP      Eth1/35(P)   Eth1/36(P)
----
. 기존 클러스터 스위치에 연결된 각 노드의 클러스터 포트 'network device-discovery show'를 표시합니다
+
[listing, subs="+quotes"]
----
...
+
.. 모든 클러스터 포트가 정상 상태인지 'network port show-ipSpace Cluster'인지 확인합니다
+
[listing, subs="+quotes"]
----
...
e0b       Cluster      Cluster          up   9000  auto/10000 healthy  false
----
.. 모든 클러스터 인터페이스(LIF)가 홈 포트 'network interface show-vserver Cluster'에 있는지 확인합니다
+
[listing, subs="+quotes"]
----
...
----
. 클러스터 LIF가 클러스터 스위치 CS1에 호스팅된 포트로 마이그레이션되었는지 확인합니다. 네트워크 인터페이스 show-vserver Cluster가 몇 초 정도 걸릴 수 있습니다
+
[listing, subs="+quotes"]
----
cluster1::*> *network interface show -vserver Cluster*
...
----
. 모든 클러스터 노드 연결을 이전 CS2 스위치에서 새 CS2-새 스위치로 이동합니다.
+
* 클러스터 노드 연결이 CS2-새 스위치로 이동됨 *

+
image::../media/new_switch_cs1.png[클러스터 노드 연결이 CS2-새 스위치로 이동했습니다]

. CS2로 이동한 네트워크 연결 상태 확인 - NEW: 네트워크 포트 표시 - IPSpace 클러스터
+
[listing, subs="+quotes"]
----
cluster1::*> *network port show -ipspace Cluster*
...
. 클러스터 포트의 네이버 정보 'network device-discovery show-protocol CDP'를 확인한다
+
[listing, subs="+quotes"]
----
cluster1::*> *network device-discovery show -protocol cdp*
...
. 클러스터 LIF가 스위치 CS2에서 호스팅되는 포트로 마이그레이션되었는지 확인합니다. 네트워크 인터페이스 show-vserver Cluster가 몇 초 정도 걸릴 수 있습니다
+
[listing, subs="+quotes"]
----
cluster1::*> *network interface show -vserver Cluster*
...
node2      true    true          false
----
. CS1에서 새 CS1-새 스위치로 연결을 이동합니다.
+
* 클러스터 노드 연결이 CS1-새 스위치 * 로 이동되었습니다

+
image::../media/new_switch_cs2.png[클러스터 노드 연결이 CS1-새 스위치로 이동했습니다]

. CS1로 이동한 네트워크 연결의 상태를 확인합니다. 새 네트워크 포트 표시 - IPSpace 클러스터
+
[listing, subs="+quotes"]
----
cluster1::*> *network port show -ipspace Cluster*
...
. 클러스터 포트에 대한 네이버 정보 확인: 네트워크 디바이스 검색 쇼
+
[listing, subs="+quotes"]
----
cluster1::*> *network device-discovery show -protocol cdp*
...
----
. CS1-NEW와 CS2-NEW 사이의 ISL이 여전히 작동 중인지 확인합니다. '포트-채널 요약 정보'
+
[listing, subs="+quotes"]
----
cs1-new# *show port-channel summary*
...
----
. 클러스터 LIF가 홈 포트로 되돌려졌는지 확인합니다(1분 정도 걸릴 수 있음). 'network interface show -vserver Cluster'
+
클러스터 LIF가 홈 포트로 되돌리지 않은 경우 수동으로 '네트워크 인터페이스 되돌리기 - vserver Cluster-lif *'를 되돌립니다

. 클러스터 상태가 양호한지 확인합니다. 'cluster show'
. 원격 클러스터 인터페이스를 ping하여 'cluster ping-cluster-node <name>'의 연결을 확인합니다
+
[listing, subs="+quotes"]
----
...
--
====
. [[27단계] 자동 케이스 생성을 억제한 경우 AutoSupport 메시지 '시스템 노드 AutoSupport invoke -node * -type all-message MAINT=end'를 호출하여 다시 활성화합니다
