...
----
. 포트 e1/33-34의 CS1에서 C1과 CS1 사이에 임시 ISL을 구성합니다.
+
...
cs2(config)# *exit*
----
. 포트 1/33-34에서 ISL 케이블을 C2에서 분리하고 케이블을 CS2의 포트 1/33-34에 연결합니다.
. ISL 포트와 포트 채널이 C2와 CS2를 연결하는 작동 중인지 확인합니다. '포트-채널 요약 정보 확인
+
다음 예에서는 ISL 포트가 C2 및 CS2에서 작동하는지 확인하는 데 사용되는 Cisco show port-channel summary 명령을 보여 줍니다.
