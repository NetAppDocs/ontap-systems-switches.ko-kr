...
----
====
. EPLD 이미지를 업그레이드하고 스위치를 재부팅합니다. NX-OS 10.5(3)F 이상을 사용하는 경우 EPLD 이미지가 NX-OS 이미지에 포함되어 있으므로 별도의 EPLD 이미지 업그레이드를 수행할 필요가 없습니다. 이 단계를 건너뛰고 클러스터 포트 상태 확인을 계속하십시오.
+
.예를 보여주세요
[%collapsible]
====
...
--
====
. [[step16]]클러스터 LIF에서 자동 되돌리기를 활성화합니다.
+
...
.다음은 무엇인가요?
NX-OS 소프트웨어를 설치하거나 업그레이드한 후에는 다음을 수행할 수 있습니다.link:install-upgrade-rcf-overview-cluster.html["참조 구성 파일(RCF) 설치 또는 업그레이드"] .
