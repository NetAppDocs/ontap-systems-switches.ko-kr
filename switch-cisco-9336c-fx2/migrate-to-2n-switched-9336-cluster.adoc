...
summary: Cisco Nexus 9336C-FX2 및 9336C-FX2-T 클러스터 스위치를 사용하여 기존의 2노드 스위치가 없는 클러스터를 스위치 클러스터 환경으로 마이그레이션합니다. 
---
= Cisco Nexus 9336C-FX2 클러스터 스위치를 사용하여 2노드 스위치 클러스터로 마이그레이션
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
기존의 2노드 스위치가 없는 클러스터를 Cisco Nexus 9336C-FX2 및 9336C-FX2-T 클러스터 스위치를 사용하는 스위치 클러스터 환경으로 마이그레이션합니다. 이 무중단 마이그레이션을 통해 클러스터를 2개 이상의 노드로 확장할 수 있습니다.
...
사용 중인 플랫폼의 클러스터 포트에 대한 정보는 link:https://hwu.netapp.com["Hardware Universe"^]를 참조하십시오. 스위치 설치 요구 사항에 대한 자세한 내용은 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU에 없는 추가 정보 중 장비를 설치하는 데 필요한 정보는 무엇입니까?"^]를 참조하십시오.
...
[%collapsible]
====
다음 예시는 node1과 node2에서 모든 LIF가 활성화되어 있음을 보여줍니다.
