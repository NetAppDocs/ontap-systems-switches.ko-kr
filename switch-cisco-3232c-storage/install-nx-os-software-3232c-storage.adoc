...
summary: Cisco Nexus 3232C 스토리지 스위치에 NX-OS 소프트웨어를 설치하거나 업그레이드합니다. 
---
= Cisco Nexus 3232C 스토리지 스위치에 NX-OS 소프트웨어 설치 또는 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 3232C 스토리지 스위치에 NX-OS 소프트웨어를 설치하거나 업그레이드하여 스토리지 네트워크 운영을 위한 준비를 완료하십시오.
...
. NX-OS 소프트웨어와 EPLD 이미지를 Nexus 3232C 스위치에 복사합니다.
+
.예를 보여주세요
[%collapsible]
====
...
----
====
. NX-OS 버전 9.3(11)으로 업그레이드하는 경우 EPLD `golden` 이미지를 업그레이드하고 스위치를 다시 한 번 재부팅해야 합니다. 그렇지 않으면 12단계로 건너뛰십시오.
+
자세한 내용은  https://www.cisco.com/c/en/us/td/docs/switches/datacenter/nexus9000/sw/93x/epld-rn/nxos_n9K_epldRN_9311.html["EPLD 업그레이드 릴리스 노트, 릴리스 9.3(11)"^]을 참조하십시오.
...
----
====
. 스위치를 처음 설정하는 경우 15단계로 건너뛰십시오. 스위치를 업그레이드하는 경우 다음 단계로 진행하십시오.
...
.다음은 무엇인가요?
NX-OS 소프트웨어를 설치한 후 link:install-rcf-software-3232c-storage.html["참조 구성 파일(RCF) 설치 또는 업그레이드"]할 수 있습니다.
