...
* 클러스터가 완전하게 작동해야 합니다(로그에 오류가 없거나 유사한 문제가 없어야 함).
...
* 의 스위치 호환성 표를 참조하십시오 link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco 이더넷 스위치 페이지"^] 지원되는 ONTAP, NX-OS 및 RCF 버전에 적합합니다.
* RCF의 명령 구문과 NX-OS 버전에 있는 명령 구문 간에 명령 의존성이 있을 수 있습니다.
* 의 Cisco 스위치 업그레이드 및 다운그레이드 절차에 대한 전체 문서를 보려면 Cisco 웹 사이트에서 제공되는 적절한 소프트웨어 및 업그레이드 가이드를 참조해야 합니다 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 시리즈 스위치"^].
* 현재 RCF가 있어야 합니다.
