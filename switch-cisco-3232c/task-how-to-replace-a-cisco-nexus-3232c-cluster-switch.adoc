...
* * 클러스터 포트를 표시 및 스위치 C2로 마이그레이션(단계 ,1-,7)*
* * 스위치 CL2에서 스위치 C2로 ISL 케이블을 다시 연결한 다음 ISL을 스위치 CL1 및 C2로 마이그레이션합니다(단계 ,8-,14)*
* * 모든 LIF를 원래 할당된 포트(단계)로 되돌리기 ,15-,18)*
* * 모든 포트와 LIF가 올바르게 마이그레이션되었는지 확인(단계 ,19-,21) *
