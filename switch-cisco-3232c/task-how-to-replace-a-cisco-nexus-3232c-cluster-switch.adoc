...
* * 클러스터 포트를 표시 및 스위치 C2로 마이그레이션(단계 <<one,1>>-<<seven,7)>>*
* * 스위치 CL2에서 스위치 C2로 ISL 케이블을 다시 연결한 다음 ISL을 스위치 CL1 및 C2로 마이그레이션합니다(단계 <<eight,8>>-<<fourteen,14)>>*
* * 모든 LIF를 원래 할당된 포트(단계)로 되돌리기 <<fifteen,15>>-<<eighteen,18)>>*
* * 모든 포트와 LIF가 올바르게 마이그레이션되었는지 확인(단계 <<nineteen,19>>-<<twentyone,21>>) *
...
이 절차의 예에서는 4개의 노드를 사용합니다. 노드 중 2개에서 10GB 클러스터 인터커넥트 포트 4개, e0a, e0b, e0c, e0d를 사용합니다. 다른 두 노드는 두 개의 40GB 클러스터 인터커넥트 포트(e4a 및 e4e)를 사용합니다. 를 참조하십시오 link:https://hwu.netapp.com/SWITCH/INDEX.html["Hardware Universe"^] 사용 중인 플랫폼에 맞는 클러스터 포트를 확인합니다.
...
----
. [[7]] 원격 클러스터 인터페이스에 대해 Ping을 수행하고 RPC 서버 검사를 수행합니다.
+
...
----
. [[8]] 클러스터 스위치 CL1의 포트 1/31 및 1/32 를 종료합니다.
+
...
1     Po1(SU)      Eth    LACP      Eth1/31(P)   Eth1/32(P)
----
. [[14]] 모든 노드에서 교체 스위치 C2:'network port modify -node_node -name_-port_port -name_-up-admin true'에 연결된 모든 클러스터 상호 연결 포트를 불러옵니다
+
[listing]
----
...
----
. [[15]] 마이그레이션된 모든 클러스터 인터커넥트 LIF를 모든 노드에서 되돌리기:
+
...
e4e       Cluster      Cluster          up   9000 auto/40000  -
        12 entries were displayed.
----
. [[18]] 원격 클러스터 인터페이스에 대해 Ping을 수행하고 RPC 서버 검사를 수행합니다.
+
'cluster ping-cluster-node_node-name_'
...
8	paths up, 0 paths down (udp check)
----
. [[19]] 다음 명령을 입력하여 구성의 장치에 대한 정보를 표시합니다.
+
...
. [[twentyone]] 적절한 클러스터 스위치가 모니터링되는지 확인합니다. 'system cluster-switch show'
+
