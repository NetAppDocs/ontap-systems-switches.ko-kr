...
summary: 결함이 있는 Cisco Nexus 3232C 클러스터 스위치를 교체합니다. 이 작업은 시스템 중단 없이 진행됩니다. 
---
...
[role="lead"]
클러스터 운영을 중단하지 않고 결함이 있는 Cisco Nexus 3232C 클러스터 스위치를 교체합니다. 이 비중단 절차는 교체 스위치를 준비하고 결함이 있는 스위치에서 연결을 마이그레이션하는 과정을 포함합니다.
...
.필요한 것
기존 클러스터 및 네트워크 구성이 다음과 같은 특징을 갖추고 있는지 확인하십시오.

* Nexus 3232C 클러스터 인프라는 두 스위치 모두에서 이중화되어 완벽하게 작동합니다.
+
Cisco 이더넷 스위치 페이지에는 스위치에 설치된 최신 RCF 및 NX-OS 버전이 있습니다.

* 모든 클러스터 포트는 *업* 상태여야 합니다.
* 두 스위치 모두에 관리 연결이 있어야 합니다.
* 모든 클러스터 논리 인터페이스(LIF)는 *업* 상태이며 마이그레이션되지 않았습니다.


교체용 Cisco Nexus 3232C 스위치의 특징은 다음과 같습니다.
...
* 클러스터는 초기에 두 개의 Nexus 3232C 클러스터 스위치 CL1과 CL2에 연결된 네 개의 노드로 구성됩니다.
* 클러스터 스위치 CL2를 C2로 교체할 계획입니다(1단계~21단계).
+
** 각 노드에서 클러스터 스위치 CL2에 연결된 클러스터 LIF를 클러스터 스위치 CL1에 연결된 클러스터 포트로 마이그레이션합니다.
** 클러스터 스위치 CL2의 모든 포트에서 케이블을 분리한 다음, 교체용 클러스터 스위치 C2의 동일한 포트에 케이블을 다시 연결합니다.
...
.예시에 관하여
이 교체 절차는 두 번째 Nexus 3232C 클러스터 스위치 CL2를 새 3232C 스위치 C2로 교체합니다.

이 절차의 예에서는 다음 스위치 및 노드 명명법을 사용합니다.

* 네 개의 노드는 n1, n2, n3, n4입니다.
* n1_clus1은 노드 n1에 대해 클러스터 스위치 C1에 연결된 첫 번째 클러스터 논리 인터페이스(LIF)입니다.
* n1_clus2는 노드 n1에 대해 클러스터 스위치 CL2 또는 C2에 연결된 첫 번째 클러스터 LIF입니다.
* n1_clus3는 노드 n1에 대해 클러스터 스위치 C2에 연결된 두 번째 LIF입니다.
* n1_clus4는 노드 n1에 대해 클러스터 스위치 CL1에 연결된 두 번째 LIF입니다.
...
이 교체 절차의 예시에서는 4개의 노드를 사용합니다. 이 중 2개의 노드는 10GB 클러스터 인터커넥트 포트 4개(e0a, e0b, e0c, e0d)를 사용합니다. 나머지 2개의 노드는 40GB 클러스터 인터커넥트 포트 2개(e4a, e4e)를 사용합니다. 사용 중인 플랫폼에 맞는 올바른 클러스터 포트를 확인하려면 link:https://hwu.netapp.com["Hardware Universe"^]을(를) 참조하십시오.



[[step-1-display-and-migrate-the-cluster-ports-to-switch]]
=== 1단계: 클러스터 포트를 스위치로 표시하고 마이그레이션합니다
...
. 새 Nexus 3232C 스위치에 적절한 RCF 및 이미지가 설치되었는지 확인하고 필요한 사이트 사용자 지정을 수행하십시오.
+
.. NetApp 지원 사이트로 이동하세요.
+
http://mysupport.netapp.com/["mysupport.netapp.com"^]

.. *Cisco 이더넷 스위치* 페이지로 이동하여 표에서 필요한 소프트웨어 버전을 확인하십시오.
+
https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco 이더넷 스위치"^]

.. RCF의 적절한 버전을 다운로드하십시오.
.. *설명* 페이지에서 *계속*을 클릭하고 사용권 계약에 동의한 다음 *다운로드* 페이지로 이동하세요.
...
[%collapsible]
====
다음 예시와 같이 모든 클러스터 LIF를 개별적으로 마이그레이션해야 합니다.
...
[%collapsible]
====
다음 예시는 클러스터 인터커넥트 포트가 모든 노드에서 차단된 경우를 보여줍니다.
...
=== 2단계: ISL을 스위치 CL1 및 C2로 마이그레이션

. 클러스터 스위치 CL1의 1/31번 포트와 1/32번 포트를 종료하십시오.
+
...
----
====
. 클러스터 스위치 CL2에 연결된 모든 케이블을 제거하고 모든 노드의 교체 스위치 C2에 다시 연결하십시오.
. 클러스터 스위치 CL2의 포트 e1/31 및 e1/32에서 스위치 간 링크(ISL) 케이블을 제거하고 교체 스위치 C2의 동일한 포트에 다시 연결하십시오.
. 클러스터 스위치 CL1에서 ISL 포트 1/31 및 1/32를 활성화하십시오.
+
Cisco 명령에 대한 자세한 내용은 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-command-reference-list.html["Cisco Nexus 3000 시리즈 NX-OS 명령어 참조"^]에 나열된 가이드를 참조하십시오.
...
----
====
. CL1에서 ISL이 활성화되었는지 확인하십시오.
+
Cisco 명령에 대한 자세한 내용은 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-command-reference-list.html["Cisco Nexus 3000 시리즈 NX-OS 명령어 참조"^]에 나열된 가이드를 참조하십시오.

+
Eth1/31 및 Eth1/32 포트에  `(P)`가 표시되어야 하며, 이는 ISL 포트가 포트 채널에서 활성화되어 있음을 의미합니다.
...
----
====
. 클러스터 스위치 C2에서 ISL이 활성화되어 있는지 확인하십시오.
+
Cisco 명령에 대한 자세한 내용은 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-command-reference-list.html["Cisco Nexus 3000 시리즈 NX-OS 명령어 참조"^]에 나열된 가이드를 참조하십시오.
...
[%collapsible]
====
Eth1/31 및 Eth1/32 포트에는 (P)가 표시되어야 합니다. 이는 두 ISL 포트가 포트 채널에서 모두 활성화되어 있음을 의미합니다.
...
----
====
. 모든 노드에서 교체 스위치 C2에 연결된 모든 클러스터 인터커넥트 포트를 활성화하십시오.
+
...
=== 3단계: 모든 LIF를 원래 할당된 포트로 되돌립니다.

. 모든 노드에서 마이그레이션된 클러스터 인터커넥트 LIF를 모두 되돌립니다.
+
...
[%collapsible]
====
다음 예시와 같이 모든 클러스터 인터커넥트 LIF를 개별적으로 되돌려야 합니다.
...
----
====
. 클러스터 인터커넥트 포트가 홈으로 복원되었는지 확인하십시오.
+
`network interface show`
...
[%collapsible]
====
다음 예는  `Current Port` 열 아래에 나열된 포트가  `Is Home` 열에서  `true` 상태를 나타내므로 모든 LIF가 성공적으로 되돌려졌음을 보여줍니다. 포트 값이  `false`인 경우 LIF가 되돌려지지 않은 것입니다.
...
. 다음 명령어를 입력하여 구성에 있는 장치에 대한 정보를 표시하십시오.
+
다음 명령어는 순서에 상관없이 실행할 수 있습니다.
...
----
====
. 교체된 클러스터 스위치 CL2가 자동으로 삭제되지 않은 경우 삭제하십시오.
+
...
[%collapsible]
====
다음 예는 클러스터 스위치가 모니터링되고 있음을 보여줍니다. 왜냐하면 `Is Monitored` 상태가 `true`이기 때문입니다.
