...
이 교체 절차의 예에서는 4개의 노드를 사용합니다. 노드 중 2개에서 10GB 클러스터 인터커넥트 포트 4개, e0a, e0b, e0c, e0d를 사용합니다. 다른 두 노드는 두 개의 40GB 클러스터 인터커넥트 포트(e4a 및 e4e)를 사용합니다. 를 참조하십시오 link:https://hwu.netapp.com/SWITCH/INDEX.html["Hardware Universe"^] 사용 중인 플랫폼에 맞는 클러스터 포트를 확인합니다.
