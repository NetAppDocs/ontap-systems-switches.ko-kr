...
[role="lead"]
ONTAP 클러스터의 이전 Cisco 스위치에서 NVIDIA SN2100 스토리지 스위치로 마이그레이션할 수 있습니다. 이는 무중단으로 수행할 수 있는 절차입니다.



== 요구사항 검토

일부 구형 Cisco 스위치를 NVIDIA SN2100 스토리지 스위치로 교체하는 경우 특정 구성 정보, 포트 연결 및 케이블 연결 요구 사항을 알고 있어야 합니다. 을 참조하십시오 link:configure-overview-sn2100-cluster.html["NVIDIA SN2100 스위치의 설치 및 구성 개요"].
...
지원되는 포트 및 해당 구성에 대한 자세한 내용은 를 참조하십시오 https://["Hardware Universe"^].
...
* NVIDIA SN2100 스위치의 일부 포트는 100GbE로 실행되도록 구성되어 있습니다.
...
----
====
. 각 노드의 스토리지 포트는 명령을 사용하여 다음과 같은 방법으로 기존 스토리지 스위치에 연결됩니다.
+
...
----
====
. 스위치 S1 및 S2에서 스토리지 포트와 스위치는 다음과 같은 방식으로(스위치의 관점에서) 연결됩니다.
+
`show lldp neighbors`
