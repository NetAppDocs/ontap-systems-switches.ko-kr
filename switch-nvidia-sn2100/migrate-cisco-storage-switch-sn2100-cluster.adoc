...
summary: 기존 Cisco 스토리지 스위치에서 NVIDIA SN2100 스토리지 스위치로 스토리지 네트워크 운영을 중단하지 않고 마이그레이션합니다. 
---
...
[role="lead"]
기존 Cisco 스토리지 스위치에서 NVIDIA SN2100 스토리지 스위치로 스토리지 네트워크 운영을 중단하지 않고 마이그레이션합니다.
...
일부 기존 Cisco 스위치를 NVIDIA SN2100 스토리지 스위치로 교체할 때는 특정 구성 정보, 포트 연결 및 케이블 요구 사항을 숙지해야 합니다. link:configure-overview-sn2100-cluster.html["NVIDIA SN2100 스위치 설치 및 구성 개요"]을(를) 참조하십시오.
