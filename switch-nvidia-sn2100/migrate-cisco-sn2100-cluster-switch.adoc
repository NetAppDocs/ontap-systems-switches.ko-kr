...
[role="lead"]
클러스터 운영을 중단하지 않고 Cisco 클러스터 스위치에서 NVIDIA SN2100 클러스터 스위치로 마이그레이션하십시오.
...
기존 Cisco 클러스터 스위치를 NVIDIA SN2100 클러스터 스위치로 교체할 때는 특정 구성 정보, 포트 연결 및 케이블 요구 사항을 숙지해야 합니다. link:configure-overview-sn2100-cluster.html["NVIDIA SN2100 스위치 설치 및 구성 개요"]을 참조하십시오.
...
NOTE: AFF A800 또는 AFF C800 시스템에서 e0a 및 e1a 클러스터 포트의 포트 속도를 변경하는 경우 속도 변환 후 잘못된 형식의 패킷이 수신되는 것을 확인할 수 있습니다. 자세한 내용은 link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1570339["버그 1570339"^] 및 기술 자료 문서 link:https://kb.netapp.com/onprem/ontap/hardware/CRC_errors_on_T6_ports_after_converting_from_40GbE_to_100GbE["40GbE에서 100GbE로 변환 후 T6 포트에서 CRC 오류 발생"^]를 참조하십시오.
