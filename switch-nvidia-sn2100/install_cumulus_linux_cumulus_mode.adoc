...
[role="lead"]
Cumulus Linux(CL) OS는 스위치에서 Cumulus Linux 또는 ONIE를 실행할 때 설치할 수 있습니다.

.시작하기 전에
다음과 같은 가정이 이루어집니다.

* 중급 수준의 Linux 지식을 보유하고 있습니다.
* 기본적인 텍스트 편집, UNIX 파일 권한 및 프로세스 모니터링에 익숙합니다. vi와 nano 등 다양한 텍스트 편집기가 미리 설치되어 있습니다.
* Linux 또는 UNIX 셸에 대한 액세스 권한이 있어야 합니다. Windows를 실행 중인 경우, Linux 환경을 Cumulus Linux와 상호 작용하기 위한 명령줄 도구로 사용합니다.
...
.단계
. 스위치에 로그인합니다. 스위치에 처음 로그인할 때는 'SUDO' 권한이 있는 * cumulus * / * cumulus * 의 사용자 이름/암호가 필요합니다.
+
[listing, subs="+quotes"]
----
...
. 1-4단계를 반복하여 로그인합니다.
. Cumulus Linux 버전이 4.4.2인지 확인합니다.
+
[listing, subs="+quotes"]
----
...
----
. 새 사용자를 생성하고 이 사용자를 'SUDO' 그룹에 추가합니다. 이 사용자는 콘솔/SSH 세션을 다시 시작한 후에만 적용됩니다.
+
[listing, subs="+quotes"]
----
