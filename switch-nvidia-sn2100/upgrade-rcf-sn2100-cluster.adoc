...
summary: 'NVIDIA SN2100 클러스터 스위치에서 새로운 버전의 참조 구성 파일(RCF) 스크립트가 출시되면 해당 스크립트를 업그레이드하십시오.' 
---
= NVIDIA SN2100 클러스터 스위치의 RCF 스크립트 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVIDIA SN2100 클러스터 스위치에서 새로운 버전의 참조 구성 파일(RCF) 스크립트가 출시되면 해당 스크립트를 업그레이드하십시오.

.시작하기 전에
RCF 스크립트를 업그레이드하기 전에 스위치에서 다음 사항을 사용할 수 있는지 확인하세요.
...
NOTE: 하지만 `scp` 예시에서는 원하는 파일 전송 방법을 사용하면 됩니다.
...
NOTE: 위의 3단계 *MOTD 파일 업데이트*에서 명령 `cat /etc/issue` 실행됩니다.  이를 통해 RCF 배너의 RCF 파일 이름, RCF 버전, 사용할 포트 및 기타 중요 정보를 확인할 수 있습니다.
...
NOTE: 하는 동안 `scp` 예시에서는 원하는 파일 전송 방법을 사용하면 됩니다.
