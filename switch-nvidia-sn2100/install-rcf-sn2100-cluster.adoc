...
* Cumulus Linux가 설치되었습니다.  를 참조하십시오 https://hwu.netapp.com/Switch/Index["Hardware Universe"^] 지원되는 버전에 대해서는.
...
| 무리 | 4x10GbE 브레이크아웃으로 구성된 포트에서 한 포트는 4x25GbE 브레이크아웃으로 구성되고 다른 포트는 40/100GbE로 구성됩니다. 공유 클러스터/HA 포트를 사용하는 노드의 포트에서 공유 클러스터/HA 트래픽을 지원합니다. 지식 기반 문서에서 플랫폼 테이블을 확인하세요. https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["어떤 AFF, ASA, FAS 플랫폼이 공유 클러스터 및 HA 이더넷 포트를 사용합니까?"^] . 모든 포트는 전용 클러스터 포트로도 사용할 수 있습니다. 
