...
summary: 'NVIDIA SN2100 클러스터 스위치에 참조 구성 파일(RCF) 스크립트를 설치하거나 업그레이드합니다.' 
---
= NVIDIA SN2100 클러스터 스위치에 RCF 스크립트 설치 또는 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVIDIA SN2100 클러스터 스위치에서 포트 할당 및 네트워크 설정을 구성하려면 참조 구성 파일(RCF) 스크립트를 설치하거나 업그레이드하십시오.

.시작하기 전에
RCF 스크립트를 설치하거나 업그레이드하기 전에 스위치에서 다음 사항을 사용할 수 있는지 확인하세요.

* Cumulus Linux가 설치되었습니다. 지원되는 버전은 link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^]를 참조하십시오.
* IP 주소, 서브넷 마스크, 기본 게이트웨이는 DHCP를 통해 정의되거나 수동으로 구성됩니다.



NOTE: 로그 수집에 특별히 사용할 사용자를 RCF에 지정해야 합니다(관리자 사용자 외에).

.고객 구성
사용 가능한 참조 구성 범주는 다음과 같습니다.

[cols="1,2"]
|===


| 무리 | 4x10GbE 브레이크아웃으로 구성된 포트에서 한 포트는 4x25GbE 브레이크아웃으로 구성되고 다른 포트는 40/100GbE로 구성됩니다. 공유 클러스터/HA 포트를 사용하는 노드의 포트에서 공유 클러스터/HA 트래픽을 지원합니다. Knowledge Base 문서 link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["어떤 AFF, ASA, FAS 플랫폼이 공유 클러스터 및 HA 이더넷 포트를 사용합니까?"^]의 플랫폼 표를 참조하십시오. 모든 포트를 전용 클러스터 포트로도 사용할 수 있습니다. 


 a| 
스토리지
 a| 
모든 포트는 100GbE NVMe 스토리지 연결을 위해 구성되었습니다.

|===
.현재 RCF 스크립트 버전
클러스터 및 스토리지 애플리케이션에 사용할 수 있는 RCF 스크립트는 두 가지가 있습니다.  RCF 다운로드 https://mysupport.netapp.com/site/info/nvidia-cluster-switch["NVIDIA SN2100 소프트웨어 다운로드"^] 페이지.  각각의 절차는 동일합니다.

* 클러스터: *MSN2100-RCF-v1._x_-클러스터-HA-브레이크아웃-LLDP*
* 저장 장치: *MSN2100-RCF-v1._x_-저장 장치*


.예시에 관하여
다음 예제 절차에서는 클러스터 스위치에 대한 RCF 스크립트를 다운로드하고 적용하는 방법을 보여줍니다.

예제 명령 출력에서는 스위치 관리 IP 주소 10.233.204.71, 넷마스크 255.255.254.0, 기본 게이트웨이 10.233.204.1을 사용합니다.
...
. 사용하다 `ping` Cumulus Linux와 RCF를 호스팅하는 서버에 대한 연결을 확인하는 명령입니다.
...
** Cumulus Linux 버전을 방금 업그레이드한 경우 자동 되돌리기가 이미 비활성화되어 있으므로 이 단계에서는 자동 되돌리기를 비활성화할 필요가 없습니다.


====
. SN2100 스위치에서 사용 가능한 인터페이스를 표시합니다.
+
[listing, subs="+quotes"]
----
...
NOTE: 하는 동안 `scp` 예시에서는 SFTP, HTTPS 또는 FTP 등 원하는 파일 전송 방법을 사용할 수 있습니다.

. RCF 파이썬 스크립트 *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP*를 적용합니다.
+
[listing, subs="+quotes"]
----
...
NOTE: 위의 3단계 *MOTD 파일 업데이트*에서 명령 `cat /etc/motd` 실행됩니다.  이를 통해 RCF 배너의 RCF 파일 이름, RCF 버전, 사용할 포트 및 기타 중요 정보를 확인할 수 있습니다.

+

NOTE: 수정할 수 없는 RCF 파이썬 스크립트 문제가 있는 경우 문의하세요. https://mysupport.netapp.com/["NetApp 지원"^] 도움이 필요하면.

. 스위치 구성에 이전의 사용자 정의를 다시 적용합니다.  참조하다link:cabling-considerations-sn2100-cluster.html["케이블링 및 구성 고려 사항 검토"] 추가로 필요한 변경 사항에 대한 자세한 내용은 문의하세요.
. 재부팅 후 구성을 확인하세요.
+
[listing, subs="+quotes"]
----
...
e3b       Cluster      Cluster          up   9000  auto/10000 healthy  false
----
.. 클러스터에서 스위치 상태를 확인합니다(LIF가 e0d에 위치하지 않으므로 스위치 sw2가 표시되지 않을 수 있음).
+
[listing, subs="+quotes"]
----
...
NOTE: 위의 3단계 *MOTD 파일 업데이트*에서 명령 `cat /etc/issue.net` 실행됩니다.  이를 통해 RCF 배너의 RCF 파일 이름, RCF 버전, 사용할 포트 및 기타 중요 정보를 확인할 수 있습니다.
...
NOTE: 나열된 예외는 성능에 영향을 미치지 않으므로 무시해도 됩니다.
...
NOTE: 하지만 `scp` 예시에서는 SFTP, HTTPS 또는 FTP 등 원하는 파일 전송 방법을 사용할 수 있습니다.
...
NOTE: 위의 3단계 *MOTD 파일 업데이트*에서 `cat /etc/issue.net` 명령이 실행됩니다.  이를 통해 RCF 배너의 RCF 파일 이름, RCF 버전, 사용할 포트 및 기타 중요 정보를 확인할 수 있습니다.
...
====
.다음은 무엇인가요?
RCF를 설치한 후에는 다음을 수행할 수 있습니다. link:setup-install-cshm-file.html["CSHM 파일을 설치하세요"].
