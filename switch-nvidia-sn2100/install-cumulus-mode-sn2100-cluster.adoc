...
summary: NVIDIA SN2100 클러스터 스위치가 Cumulus 모드로 실행 중일 때 Cumulus Linux를 설치하십시오. 
---
= NVIDIA SN2100 클러스터 스위치에 Cumulus Linux를 Cumulus 모드로 설치합니다.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVIDIA SN2100 클러스터 스위치가 Cumulus 모드로 실행 중일 때 Cumulus Linux(CL)를 설치합니다.


NOTE: Cumulus Linux(CL) OS는 스위치가 Cumulus Linux 또는 ONIE를 실행 중일 때 설치할 수 있습니다(link:install-onie-mode-sn2100-cluster.html["ONIE 모드로 설치"] 참조).
...
----
+
자세한 내용은  https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-54/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA 사용자 계정"^]을(를) 참조하십시오.
...
자세한 내용은  https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-511/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA 사용자 계정"^]을(를) 참조하십시오.

--
====
.다음은 무엇인가요?
Cumulus 모드로 Cumulus Linux를 설치한 후 link:install-rcf-sn2100-cluster.html["참조 구성 파일(RCF) 스크립트 설치"].
