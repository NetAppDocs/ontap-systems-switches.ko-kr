...
summary: NVIDIA SN2100 클러스터 스위치를 배포하기 전에 케이블 연결 및 구성 고려 사항을 검토하십시오. 
---
= NVIDIA SN2100 클러스터 스위치의 케이블 연결 및 구성 고려 사항을 검토하십시오.
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVIDIA SN2100 클러스터 스위치를 배포하기 전에 케이블 연결 및 구성 고려 사항을 검토하십시오.
...
|===
스위치 포트에 대한 자세한 내용은 link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^]을(를) 참조하십시오.
...
--
====
자세한 내용은 기술 자료 문서 link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Systems/Fabric_Interconnect_and_Management_Switches/NVIDIA_SN2100_switch_fails_to_connect_using_40_100GbE_copper_cable["SN2100 스위치가 40/100GbE 구리 케이블을 사용하여 연결에 실패합니다."^]를 참조하십시오.
...
QSFP+ (40GbE)-SFP+ (10GbE) 어댑터 또는 QSFP28 (100GbE)-SFP28 (25GbE) 어댑터(QSA)를 사용할 때는 비분리형 40GbE/100GbE 스위치 포트(swp3-swp14)에 삽입하십시오. 브레이크아웃용으로 구성된 포트에 QSA 모듈을 삽입하지 마십시오.

QSA 모듈을 사용하여 플랫폼의 10GbE/25GbE 클러스터 포트에 연결할 때 링크가 설정되지 않을 수 있습니다.

이 문제를 해결하려면 다음을 수행하세요.

* 10GbE의 경우 링크 속도를 수동으로 10000으로 설정하고 자동 협상을 끄십시오.
* 25GbE의 경우 링크 속도를 수동으로 25000으로 설정하고 자동 협상을 끄십시오.
...
스위치 포트의 트랜시버에 따라 스위치 인터페이스의 속도를 고정 속도로 설정해야 할 수도 있습니다. 10GbE 및 25GbE 브레이크아웃 포트 또는 QSA 모듈을 사용하는 경우 자동 협상이 꺼져 있는지 확인하고 스위치에서 인터페이스 속도를 설정하십시오.
...
--
====
자세한 내용은 link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^] 및 기술 자료 문서 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU에 나와 있지 않은 추가 정보는 무엇이며, 장비를 설치하는 데 필요한 정보는 무엇입니까?"^]를 참조하십시오.
