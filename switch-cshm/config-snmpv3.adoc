...
이더넷 스위치 상태 모니터(CSHM)는 SNMP를 활용하여 클러스터 및 스토리지 스위치의 상태와 성능을 모니터링합니다.  기본적으로 SNMPv2c는 참조 구성 파일(RCF)을 통해 자동으로 구성됩니다.  SNMPv3는 인증, 암호화, 메시지 무결성과 같은 강력한 보안 기능을 도입하여 무단 접근을 방지하고 전송 중 데이터의 기밀성과 무결성을 보장하므로 SNMPv2보다 보안성이 더 높습니다.
