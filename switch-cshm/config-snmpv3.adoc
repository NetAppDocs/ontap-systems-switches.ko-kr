...
summary: '네트워크 스위치에서 SNMPv3을 구성하여 이더넷 스위치 상태 모니터링(CSHM)과의 안전한 통신을 활성화하십시오.' 
---
= 이더넷 스위치 상태 모니터링을 위한 SNMPv3 구성
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
네트워크 스위치에서 SNMPv3을 구성하여 이더넷 스위치 상태 모니터링(CSHM)과의 안전하고 인증된 통신을 활성화하십시오. SNMPv3은 ONTAP에서 상태 모니터링을 위한 스위치 상태의 암호화된 폴링을 제공합니다.

이더넷 스위치 상태 모니터(CSHM)는 SNMP를 사용하여 네트워크 스위치의 상태와 성능을 모니터링합니다. 기본적으로 참조 구성 파일(RCF)을 통해 SNMPv2c가 자동으로 구성됩니다. SNMPv3은 인증, 암호화 및 메시지 무결성과 같은 강력한 보안 기능을 도입하여 SNMPv2보다 보안성이 높습니다. 이러한 기능은 무단 접근을 방지하고 전송 중 데이터의 기밀성과 무결성을 보장합니다.
...
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-119131["SNMP 실패 발생 시 거짓 양성 스위치 팬 및 전원 경고"^]
...
====
.이 작업에 관하여
다음 명령은 *Broadcom*, * Cisco*, * NVIDIA* 스위치에서 SNMPv3 사용자 이름을 구성하는 데 사용됩니다.

[role="tabbed-block"]
====
.브로드컴 스위치
--
Broadcom BES-53248 스위치에서 SNMPv3 사용자 이름 NETWORK-OPERATOR를 구성합니다.
...
.Cisco 스위치
--
Cisco Nexus 스위치에서 SNMPv3 사용자 이름 SNMPv3_USER를 구성합니다.
...
.NVIDIA - CL 5.4.0
--
NVIDIA Cumulus 스위치(CL 5.4.0 실행 중)에서 SNMPv3 사용자 이름 SNMPv3_USER를 구성하십시오.
...
.NVIDIA - CL 5.11.0+
--
CLI 5.11.0 이상을 실행하는 NVIDIA Cumulus 스위치에서 SNMPv3 사용자 이름 SNMPv3_USER를 구성하십시오.
