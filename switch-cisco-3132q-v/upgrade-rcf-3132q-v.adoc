...
summary: 'Cisco Nexus 3132Q-V 클러스터 스위치의 참조 구성 파일(RCF)에 새 버전이 출시되면 해당 파일을 업그레이드하십시오.' 
---
= Cisco Nexus 3132Q-V 클러스터 스위치의 RCF 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 3132Q-V 클러스터 스위치에서 새로운 버전의 참조 구성 파일(RCF)이 제공되면 해당 RCF(참조 구성 파일)를 업그레이드하십시오.
...
NOTE: ONTAP 9.8 이상 버전에서는 명령어 `system switch ethernet show -is-monitoring-enabled-operational true`를 사용하십시오.
...
----
+
이 명령을 실행한 후에는 자동 되돌리기 기능이 비활성화되어 있는지 확인하십시오.
...
. 현재 실행 중인 구성과 사용 중인 RCF 파일 사이에 추가된 사용자 지정 사항을 기록하십시오.
+
[NOTE]
====
다음 사항을 구성해야 합니다.
...
NOTE: RCF를 업그레이드하거나 새 RCF를 적용할 때는 스위치 설정을 지우고 기본 구성을 수행해야 합니다.
...
+
Cisco 명령에 대한 자세한 내용은 https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 Series NX-OS Command Reference"^]의 해당 가이드를 참조하십시오.
...
NOTE: RCF 업그레이드 후 10GbE 포트를 온라인 상태로 전환하는 방법에 대한 단계는 기술 자료 문서 link:https://kb.netapp.com/onprem%2FSwitches%2FCisco%2F10GbE_ports_on_Cisco_3132Q_cluster_switch_do_not_come_online["Cisco 3132Q 클러스터 스위치의 10GbE 포트가 온라인 상태가 되지 않습니다."^]를 참조하십시오.



. RCF 버전과 스위치 설정이 올바른지 확인한 후  `running-config` 파일을  `startup-config` 파일로 복사하십시오.
+
Cisco 명령에 대한 자세한 내용은 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 Series NX-OS Command Reference"^]의 해당 가이드를 참조하십시오. .예 표시
...
----
====
. 스위치 cs2를 재부팅합니다. 스위치가 재부팅되는 동안 노드에 보고되는 "cluster ports down" 이벤트와 오류 `% Invalid command at '^' marker` 출력은 무시해도 됩니다.
+
[listing, subs="+quotes"]
----
cs2# *reload*
This command will reboot the system. (y/n)?  [n] *y*
----
. 이전에 적용했던 스위치 구성 사용자 지정을 다시 적용합니다. 추가 변경 사항에 대한 자세한 내용은 link:cabling-considerations-3132q-v.html["케이블링 및 구성 고려 사항 검토"]을 참조하십시오.
...
----
. 스위치 cs1을 재부팅하십시오. 이렇게 하면 클러스터 LIF가 원래 포트로 복귀합니다. 스위치가 재부팅되는 동안 노드에서 보고되는 "클러스터 포트 다운" 이벤트는 무시해도 됩니다.
+
[listing, subs="+quotes"]
----
cs1# *reload*
...
====
.다음은 무엇인가요?
RCF를 업그레이드한 후에는 link:configure-ssh-keys.html["SSH 구성을 확인하세요"].
