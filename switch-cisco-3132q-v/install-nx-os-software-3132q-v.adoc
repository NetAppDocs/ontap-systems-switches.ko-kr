...
[role="lead"]
다음 절차에 따라 Nexus 3132Q-V 클러스터 스위치에 NX-OS 소프트웨어를 설치합니다.
...
* https://["Cisco Nexus 3000 시리즈 스위치"^]. Cisco 스위치 업그레이드 및 다운그레이드 절차에 대한 전체 설명서는 Cisco 웹 사이트에서 제공하는 해당 소프트웨어 및 업그레이드 가이드를 참조하십시오.
...
----
====
. FTP, TFTP, SFTP 또는 SCP 중 하나의 전송 프로토콜을 사용하여 NX-OS 소프트웨어를 Nexus 3132Q-V 스위치에 복사합니다. Cisco 명령에 대한 자세한 내용은 에서 해당 설명서를 참조하십시오 https://["Cisco Nexus 3000 Series NX-OS 명령 참조 가이드"^].
+
.예제 보기
[%collapsible]
====
...
.다음 단계
link:install-rcf-3132q-v.html["RCF(Reference Configuration File) 설치"].
