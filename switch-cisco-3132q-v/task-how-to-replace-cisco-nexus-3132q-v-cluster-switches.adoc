...
---
= Cisco Nexus 3132Q-V 클러스터 스위치를 교체하는 방법
:icons: font
:imagesdir: ../media/


[role="lead"]
클러스터 네트워크에서 결함이 있는 Cisco Nexus 3132Q-V 스위치를 교체하는 것은 무중단 절차(NDO)이며 특정 작업 시퀀스를 수행해야 합니다.



== 필요한 것

* 기존 클러스터 및 네트워크 구성에는 다음이 있어야 합니다.
+
** Nexus 3132Q-V 클러스터 인프라는 두 스위치에서 모두 중복되고 완전하게 작동해야 합니다.
+
Cisco 이더넷 스위치 _ 페이지는 스위치에 최신 RCF 및 NX-OS 버전을 가지고 있습니다.
...
* Nexus 3132Q-V 교체 스위치:
+
** 교체 스위치의 관리 네트워크 연결이 작동해야 합니다.
** 교체 스위치에 대한 콘솔 액세스가 있어야 합니다.
** 원하는 RCF 및 NX-OS 운영 체제 이미지 스위치를 스위치에 로드해야 합니다.
...
이 절차는 두 번째 Nexus 3132Q-V 클러스터 스위치 CL2를 새 3132Q-V 스위치 C2로 대체합니다. 이 절차의 예에서는 다음 스위치 및 노드 명명법을 사용합니다.
...
* 노드는 n1, n2, n3 및 n4입니다. 이 절차의 예제에서는 4개의 노드를 사용합니다. 두 노드는 e0a, e0b, e0c 및 e0d 4개의 10GB 클러스터 인터커넥트 포트를 사용합니다. 다른 두 노드는 두 개의 40GB 클러스터 인터커넥트 포트(e4a 및 e4e)를 사용합니다. 플랫폼의 실제 클러스터 포트에 대한 자세한 내용은 _Hardware Universe_을 참조하십시오.


이 절차에서는 다음 시나리오에 대해 설명합니다.

* 클러스터는 2개의 Nexus 3132Q-V 클러스터 스위치인 CL1 및 CL2에 연결된 4개의 노드로 시작됩니다.
* 클러스터 스위치 CL2를 C2로 교체(1-21단계):
+
...
+
.. NetApp Support 사이트에서 다음 위치로 이동합니다. http://support.netapp.com/NOW/download/software/cm_switches/["Cisco 이더넷 스위치"^]
...
----
. CL1에서 포트 1/31 및 1/32 를 종료하고 활성 Nexus 3132Q-V 스위치를 종료합니다.
+
'허틀다운'
...
. Nexus 3132Q-V 스위치 CL1에서 ISL 포트 1/31 및 1/32 를 가져옵니다.
+
[listing]
----
...
1     Po1(SU)      Eth    LACP      Eth1/31(P)   Eth1/32(P)
----
. 모든 노드에서 Nexus 3132Q-V 스위치 C2:'네트워크 포트 수정'에 연결된 모든 클러스터 인터커넥트 포트를 불러옵니다
+
[listing]
----
