...
* 클러스터 스위치 CL2를 C2(<<step1_replace3132q,1단계>> -<<step21_replace3132q,21>>)
+
...
.단계
. [[step1_replace3132q]] 이 클러스터에서 AutoSupport가 활성화된 경우 AutoSupport 메시지를 호출하여 자동 케이스 생성을 억제합니다.
+
...
----
. [[step21_repete3132q]] 교체된 Nexus 3132Q-V 스위치가 아직 자동으로 제거되지 않은 경우 이를 제거합니다.
+
