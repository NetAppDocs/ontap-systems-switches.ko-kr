...
summary: '클러스터 네트워크에서 결함이 있는 Cisco Nexus 3132Q-V 스위치를 교체하는 것은 중단 없는 절차(NDO)이며, 특정 작업 순서를 수행해야 합니다.' 
---
...
[role="lead"]
클러스터 네트워크에서 결함이 있는 Cisco Nexus 3132Q-V 스위치를 교체하려면 다음 절차를 따르세요.  교체 절차는 중단 없는 절차(NDO)입니다.
...
.스위치 요구 사항
검토하다link:switch-requirements.html["Cisco Nexus 3132Q-V 클러스터 스위치 교체 요구 사항"] .

.시작하기 전에
* 기존 클러스터와 네트워크 구성은 다음과 같습니다.
+
** Nexus 3132Q-V 클러스터 인프라는 두 스위치 모두에서 중복되고 완벽하게 작동합니다.
+
...
** 모든 클러스터 포트는 `up` 상태.
** 두 스위치 모두에 관리 연결이 존재합니다.
** 모든 클러스터 논리 인터페이스(LIF)는 `up` 상태로 이전되었습니다.


* Nexus 3132Q-V 교체 스위치의 경우 다음 사항을 확인하세요.
+
...
이 절차에서는 두 번째 Nexus 3132Q-V 클러스터 스위치 CL2를 새로운 3132Q-V 스위치 C2로 교체합니다.
...
* n1_clus3는 노드 n1의 클러스터 스위치 C2에 연결된 두 번째 LIF입니다.
...
* 노드는 n1, n2, n3, n4입니다. - 이 절차의 예제에서는 4개의 노드를 사용합니다. 두 개의 노드는 4개의 10GB 클러스터 인터커넥트 포트(e0a, e0b, e0c, e0d)를 사용합니다. 나머지 두 개의 노드는 2개의 40GB 클러스터 인터커넥트 포트(e4a, e4e)를 사용합니다. 플랫폼의 실제 클러스터 포트는 link:https://hwu.netapp.com/["Hardware Universe"^]을 참조하십시오.


.이 작업에 관하여
이 절차는 다음 시나리오를 다룹니다.

* 클러스터는 두 개의 Nexus 3132Q-V 클러스터 스위치(CL1 및 CL2)에 연결된 네 개의 노드로 시작합니다.
* 클러스터 스위치 CL2는 C2로 교체됩니다.
+
** 각 노드에서 CL2에 연결된 클러스터 LIF는 CL1에 연결된 클러스터 포트로 마이그레이션됩니다.
** CL2의 모든 포트에서 케이블을 분리한 다음 교체 스위치 C2의 동일한 포트에 케이블을 다시 연결합니다.
...
. 요구 사항에 맞게 새로운 Nexus 3132Q-V 스위치에 적절한 RCF와 이미지가 설치되어 있는지 확인하고, 필수적인 사이트 사용자 지정을 수행합니다.
+
이때 교체 스위치를 준비해야 합니다.  RCF와 이미지를 업그레이드해야 하는 경우 다음 단계를 따라야 합니다.
...
. 스위치 C2에 연결된 클러스터 포트에 연관된 LIF를 마이그레이션합니다.
+
...
[%collapsible]
====
이 예에서는 LIF 마이그레이션이 모든 노드에서 수행된다는 것을 보여줍니다.
...
[%collapsible]
====
이 예에서는 모든 노드에서 지정된 포트가 종료되는 것을 보여줍니다.
...
--
====
. [[step9]]CL1의 포트 1/31 및 1/32와 활성 Nexus 3132Q-V 스위치를 종료합니다.
+
`shutdown`
...
[%collapsible]
====
이 예에서는 스위치 CL1에서 ISL 포트 1/31과 1/32가 종료되는 것을 보여줍니다.
...
. Nexus 3132Q-V 스위치 CL2에 연결된 모든 케이블을 제거한 다음 모든 노드의 교체 스위치 C2에 다시 연결합니다.
. CL2의 포트 e1/31 및 e1/32에서 ISL 케이블을 제거한 다음 교체 스위치 C2의 동일한 포트에 다시 연결합니다.
...
----
====
. C2에서 ISL이 작동하는지 확인하세요.
+
`show port-channel summary`

+
포트 Eth1/31 및 Eth1/32는 다음을 표시해야 합니다. `(P)` 즉, 두 ISL 포트가 모두 포트 채널에 연결되어 있다는 의미입니다.
...
----
====
. 모든 노드에서 Nexus 3132Q-V 스위치 C2에 연결된 모든 클러스터 상호 연결 포트를 불러옵니다.
+
`network port modify`
...
----
====
. 모든 노드에 대해 마이그레이션된 모든 클러스터 상호 연결 LIF를 되돌립니다.
+
...
[%collapsible]
====
이 예에서는 모든 LIF가 성공적으로 되돌려졌음을 보여줍니다. 이는 해당 포트 아래에 나열되어 있기 때문입니다. `Current Port` 열의 상태가 다음과 같습니다. `true` 에서 `Is Home` 열.  만약 `Is Home` 열 값은 `false` LIF는 복귀되지 않았습니다.
...
----
====
. 이미 자동으로 제거되지 않았다면 교체된 Nexus 3132Q-V 스위치를 제거하세요.
+
