...
summary: 결함이 있는 Cisco Nexus 3132Q-V 클러스터 스위치를 교체합니다. 이 작업은 시스템 중단 없이 진행됩니다. 
---
= Cisco Nexus 3132Q-V 클러스터 스위치 교체
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
클러스터 운영을 중단하지 않고 결함이 있는 Cisco Nexus 3132Q-V 클러스터 스위치를 교체합니다. 이 비중단 교체 절차(NDU)는 교체 스위치를 준비하고 결함이 있는 스위치에서 연결을 마이그레이션하는 과정을 포함합니다.
...
.스위치 요구 사항
link:switch-requirements.html["Cisco Nexus 3132Q-V 클러스터 스위치 교체 요구 사항"]을(를) 검토하십시오.

.시작하기 전에
* 기존 클러스터 및 네트워크 구성은 다음과 같습니다.
+
** Nexus 3132Q-V 클러스터 인프라는 이중화되어 있으며 두 스위치 모두에서 완벽하게 작동합니다.
+
link:https://mysupport.netapp.com/site/products/all/details/cisco-cluster-storage-switch/downloads-tab["Cisco 이더넷 스위치"^]스위치용 최신 RCF 및 NX-OS 버전을 제공합니다.

** 모든 클러스터 포트가  `up` 상태입니다.
** 두 스위치 모두 관리 연결이 존재합니다.
** 모든 클러스터 논리 인터페이스(LIF)가 `up` 상태에 있으며 마이그레이션이 완료되었습니다.


* Nexus 3132Q-V 교체 스위치를 설치할 때는 다음 사항을 확인하십시오.
+
...
이 절차는 두 번째 Nexus 3132Q-V 클러스터 스위치 CL2를 새 3132Q-V 스위치 C2로 교체합니다.
...
* n1_clus3은 노드 n1에 대해 클러스터 스위치 C2에 연결된 두 번째 LIF입니다.
...
* 노드는 n1, n2, n3, n4입니다. - 이 절차의 예제에서는 4개의 노드를 사용합니다. 두 개의 노드는 4개의 10GB 클러스터 인터커넥트 포트(e0a, e0b, e0c, e0d)를 사용합니다. 나머지 2개의 노드는 40GB 클러스터 인터커넥트 포트 2개(e4a, e4e)를 사용합니다. 플랫폼의 실제 클러스터 포트는 link:https://hwu.netapp.com/["Hardware Universe"^]을 참조하십시오.


.이 작업에 관하여
이 절차는 다음 시나리오를 다룹니다.

* 클러스터는 CL1과 CL2라는 두 개의 Nexus 3132Q-V 클러스터 스위치에 연결된 네 개의 노드로 시작됩니다.
* 클러스터 스위치 CL2는 C2로 교체될 예정입니다.
+
** 각 노드에서 CL2에 연결된 클러스터 LIF는 CL1에 연결된 클러스터 포트로 마이그레이션됩니다.
** CL2의 모든 포트에서 케이블을 분리하고 교체 스위치 C2의 동일한 포트에 케이블을 다시 연결하십시오.
** 각 노드에서 마이그레이션된 클러스터 LIF가 원래대로 되돌려집니다.
...
. 새 Nexus 3132Q-V 스위치에 필요한 RCF 및 이미지가 설치되었는지 확인하고, 필요한 사이트 사용자 지정을 완료하십시오.
+
이때 교체용 스위치를 준비해야 합니다. RCF 및 이미지를 업그레이드해야 하는 경우 다음 단계를 따르십시오.

+
.. NetApp 지원 사이트에서 link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch/["Cisco 이더넷 스위치"^]을(를) 참조하십시오.
...
. 스위치 C2에 연결된 클러스터 포트와 관련된 LIF를 마이그레이션합니다.
+
...
[%collapsible]
====
이 예시는 LIF 마이그레이션이 모든 노드에서 수행됨을 보여줍니다.
...
[%collapsible]
====
이 예시는 지정된 포트가 모든 노드에서 차단되는 것을 보여줍니다.
...
--
====
. [[step9]]CL1의 포트 1/31 및 1/32와 활성화된 Nexus 3132Q-V 스위치를 종료합니다.
+
`shutdown`
...
[%collapsible]
====
이 예시는 스위치 CL1에서 ISL 포트 1/31 및 1/32가 비활성화된 것을 보여줍니다.
...
. Nexus 3132Q-V 스위치 CL2에 연결된 모든 케이블을 제거하고 모든 노드의 교체 스위치 C2에 다시 연결하십시오.
. CL2의 e1/31 및 e1/32 포트에서 ISL 케이블을 분리한 다음 교체 스위치 C2의 동일한 포트에 다시 연결하십시오.
. Nexus 3132Q-V 스위치 CL1에서 ISL 포트 1/31 및 1/32를 활성화합니다.
+
[listing]
----
(CL1)# configure
...
----
====
. C2에서 ISL이 활성화되었는지 확인하십시오.
+
`show port-channel summary`

+
Eth1/31 및 Eth1/32 포트에  `(P)`가 표시되어야 하며, 이는 두 ISL 포트가 포트 채널에서 모두 활성화되어 있음을 의미합니다.
...
----
====
. 모든 노드에서 Nexus 3132Q-V 스위치 C2에 연결된 모든 클러스터 인터커넥트 포트를 활성화하십시오.
+
`network port modify`
...
----
====
. 모든 노드에 대해 마이그레이션된 클러스터 인터커넥트 LIF를 모두 되돌립니다.
+
...
[%collapsible]
====
이 예는  `Current Port` 열 아래에 나열된 포트가  `Is Home` 열에서  `true` 상태를 나타내므로 모든 LIF가 성공적으로 되돌려졌음을 보여줍니다.  `Is Home` 열 값이  `false`인 경우 LIF가 되돌려지지 않은 것입니다.
...
----
====
. 교체된 Nexus 3132Q-V 스위치가 자동으로 제거되지 않은 경우, 해당 스위치를 제거하십시오.
+
