...
---
= CN1610 클러스터 스위치를 Cisco Nexus 3132Q-V 클러스터 스위치로 교체하는 방법
:icons: font
:imagesdir: ../media/


[role="lead"]
클러스터의 기존 CN1610 클러스터 스위치를 Cisco Nexus 3132Q-V 클러스터 스위치로 교체하려면 특정 작업 시퀀스를 수행해야 합니다.



== 이 작업에 대해

이 절차의 예제에서는 4개의 노드를 사용합니다. 두 노드는 e0a, e0b, e0c 및 e0d 4개의 10GbE 클러스터 인터커넥트 포트를 사용합니다. 다른 두 노드는 40/100 GbE 클러스터 인터커넥트 파이버 케이블 2개(e4a 및 e4e)를 사용합니다. Hardware Universe_에는 플랫폼의 클러스터 파이버 케이블에 대한 정보가 있습니다.

이 절차의 예는 다음 스위치 및 노드 명명법을 사용합니다. - * 명령 출력은 ONTAP 소프트웨어의 릴리스에 따라 다를 수 있습니다. * 교체할 CN1610 스위치는 CL1 및 CL2입니다. * CN1610 스위치를 교체하기 위한 Nexus 3132Q-V 스위치는 C1 및 C2입니다. * n1_clus1 은 노드 n1의 클러스터 스위치 1(CL1 또는 C1)에 연결되는 첫 번째 클러스터 논리 인터페이스(LIF)입니다. * n1_clus2는 노드 n1의 클러스터 스위치 2(CL2 또는 C2)에 연결되는 첫 번째 클러스터 LIF입니다. * n1_clus3 은 노드 n1의 클러스터 스위치 2(CL2 또는 C2)에 연결되는 두 번째 LIF입니다. * n1_clus4 는 노드 n1의 클러스터 스위치 1(CL1 또는 C1)에 연결되는 두 번째 LIF입니다. 노드는 n1, n2, n3 및 n4입니다. * 10GbE 및 40/100 GbE 포트 수는 에서 사용할 수 있는 RCFs(참조 구성 파일)에 정의되어 있습니다 https://mysupport.netapp.com/NOW/download/software/sanswitch/fcp/Cisco/netapp_cnmn/download.shtml["Cisco ® 클러스터 네트워크 스위치 참조 구성 파일 다운로드"^] 페이지.

이 절차에서는 다음 시나리오에 대해 설명합니다.

* 클러스터가 두 CN1610 클러스터 스위치에 연결된 두 노드로 시작됩니다.
* C2로 교체할 클러스터 스위치 CL2(2단계~22단계):
+
...
+
.. 클러스터 네트워크 포트 속성 'network port show'를 표시합니다
+
...
8 entries were displayed.
----
.. 논리 인터페이스에 대한 정보를 표시한다: 'network interface show'
+
...
. 클러스터 LIF clus1 및 clus4에서 '-auto-revert' 매개 변수를 false로 설정합니다
+
[listing]
----
...
. 교체할 두 번째 CN1610 스위치에 연결된 LIF를 ' 네트워크 인터페이스 마이그레이션 ' 으로 마이그레이션합니다
+
[NOTE]
====
...
----
. 클러스터의 상태를 확인합니다. 'network interface show'
+
...
8 entries were displayed.
----
. 스위치 CL2에 물리적으로 연결된 클러스터 인터커넥트 포트를 '네트워크 포트 수정'으로 종료합니다
+
...
----
. 원격 클러스터 인터페이스에 ping을 한 다음 원격 프로시저 호출 서버 점검을 수행한다. 'cluster ping-cluster'
+
...
[NOTE]
====
새 Cisco 3132Q-V 스위치에 케이블을 다시 연결할 때는 광 파이버 또는 Cisco twinax 케이블을 사용해야 합니다.
...
(CL1)#
----
. CN1610 스위치 CL1에서 ISL이 "UP" 상태인지 확인: 'How port-channel'
+
...
0/16   actor/long    10 Gb Full  True
       partner/long
----
. 3132Q-V 스위치 C2:'포트-채널 요약 정보'에서 ISL이 "UP"인지 확인합니다
+
...
2     Po2(SU)     Eth      LACP      Eth1/24/1(P) Eth1/24/2(P) Eth1/24/3(P)
                                     Eth1/24/4(P)
----
. 모든 노드에서 3132Q-V 스위치 C2에 연결된 모든 클러스터 상호 연결 포트('네트워크 포트 수정')를 불러옵니다
+
...
----
. 모든 노드에서 C2에 연결된 마이그레이션된 모든 클러스터 상호 연결 LIF를 '네트워크 인터페이스 복원'으로 되돌립니다
+
[listing]
----
...
----
. 모든 클러스터 인터커넥트 포트가 홈 포트인 'network interface show'로 되돌려졌는지 확인합니다
+
...
8 entries were displayed.
----
. 모든 클러스터 포트가 'network port show'로 연결되어 있는지 확인합니다
+
...
4 paths up, 0 paths down (udp check)
----
. 클러스터의 각 노드에서 교체할 첫 번째 CN1610 스위치 CL1과 연결된 인터페이스를 '네트워크 인터페이스 마이그레이션'으로 마이그레이션합니다
+
...
----
. 클러스터 상태 'network interface show'를 확인한다
+
...
8 entries were displayed.
----
. 모든 노드에서 CL1에 연결된 노드 포트인 네트워크 포트 수정(network port modify)을 종료한다
+
...
----
. 모든 노드에서 새 3132Q-V 스위치 C1에 연결된 모든 클러스터 인터커넥트 포트 '네트워크 포트 수정'을 불러옵니다
+
...
8 entries were displayed.
----
. 마이그레이션된 모든 클러스터 인터커넥트 LIF를 모든 노드에서 C1에 원래 연결된 '네트워크 인터페이스 복원'으로 되돌립니다
+
...
----
. 인터페이스가 홈(network interface show)인지 확인합니다
+
...
----
. 교체된 CN1610 스위치가 자동으로 제거되지 않은 경우 '시스템 클러스터 - 스위치 삭제'를 제거하십시오
+
