...
summary: 2노드 스위치가 없는 클러스터에서 Cisco Nexus 3132Q-V 클러스터 스위치로의 마이그레이션을 완료합니다. 
---
= 2노드 스위치가 없는 클러스터에서 Cisco Nexus 3132Q-V 클러스터 스위치로의 마이그레이션을 완료합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 3132Q-V 클러스터 스위치를 사용하여 2노드 스위치가 없는 클러스터에서 2노드 스위치 클러스터로 마이그레이션하는 최종 단계를 완료하십시오.
...
----
====
. 모든 노드에서 2노드 스위치가 없는 클러스터 구성 설정을 비활성화하십시오.
+
...
----
. 해당  `switchless-cluster` 옵션이 비활성화되어 있는지 확인하십시오.
+
[listing]
----
