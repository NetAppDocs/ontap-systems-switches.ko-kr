...
summary: 'Cisco Nexus 9364D-GX2A 스위치에서 참조 구성 파일(RCF) 버전을 업그레이드하여 새로운 하드웨어, 새로운 ONTAP 버전을 지원하거나 AFX 1K 또는 AFX 2K 시스템 구성에서 새로운 NX-OS 버전에 맞게 조정합니다.' 
---
= Cisco Nexus 9364D-GX2A의 RCF 버전 업그레이드
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 9364D-GX2A 스위치에서 참조 구성 파일(RCF) 버전을 업그레이드하여 새로운 하드웨어, 새로운 ONTAP 버전을 지원하거나 AFX 1K 또는 AFX 2K 시스템 구성에서 새로운 NX-OS 버전에 맞게 조정합니다.
...
+
스위치가 RCF 10.1 이상을 실행 중인 경우 필요에 따라 link:configure-install-upgrade-rcf-overview-9364d.html#change-port-speed-configuration["포트 속도 구성 변경"] 할 수 있습니다.
...
.다음은 무엇인가요?
스위치의 RCF를 업그레이드한 후에는 link:configure-verify-ssh-9364d.html["로그 수집을 위한 SSH 확인"].
